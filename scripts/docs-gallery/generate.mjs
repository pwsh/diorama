...
const SITE_CSS = `/* GENERATED by scripts/docs-gallery/generate.mjs — DO NOT EDIT BY HAND. */
:root {
...
function htmlPage({ current, title, prefix, bodyMain, version, date }) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${htmlEsc(title)} — Diorama Model Gallery</title>
<link rel="stylesheet" href="${prefix}assets/site.css">
</head>
<body>
<header class="topbar">
<button class="menu-btn" aria-label="Toggle navigation" onclick="document.body.classList.toggle('nav-open')">☰</button>
<a class="brand" href="${prefix}index.html">Diorama — Model Gallery</a>
<span class="brand-meta">v${htmlEsc(version)} · ${htmlEsc(date)}</span>
</header>
<div class="layout">
<aside class="sidebar">${renderNav(current, prefix)}</aside>
<main class="main">${bodyMain}</main>
</div>
<script>
...
function writeHtml(subjects, version, date) {
  ...
  for (const page of pages) {
    const subs = byPage.get(page);
    const prefix = '../'.repeat((page.match(/\//g) || []).length);
    const groups = [];
    const seen = new Set();
    for (const su of subs) { if (!seen.has(su.group)) { seen.add(su.group); groups.push(su.group); } }
    ...
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, htmlPage({ current: page, title: pageTitleFor(page), prefix, bodyMain: main, version, date }));
  }
  ...
  if (NAV_AV.length) {
    ...
    fs.writeFileSync(path.join(OUT, 'avatars', 'index.html'),
      htmlPage({ current: 'avatars/index', title: 'Avatars', prefix: '../', bodyMain: main, version, date }));
  }
  ...
  fs.writeFileSync(path.join(OUT, 'index.html'),
    htmlPage({ current: 'index', title: 'Home', prefix: '', bodyMain: main, version, date }));
}
