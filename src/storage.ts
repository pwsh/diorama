...
function defaultFloor(): Floor {
  return {
    ...
    groundAreas: [], pools: [], sprinklerZones: [], voidAreas: [], infoCards: [], actionButtons: [],
    flagpoles: [], solarPanels: [], rulers: [],
  };
}
...
export function repairFloor(f: Partial<Floor> & { id: string; name: string; w: number; d: number }): Floor {
  return {
    ...
    flagpoles: f.flagpoles ?? [],
    solarPanels: f.solarPanels ?? [],
    rulers: f.rulers ?? [],
    ...
    outdoor: f.outdoor,           // bindable "everything outside the walls" pseudo-area; absent = unbound
  };
}
