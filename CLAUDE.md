...
- **AI avatars** (`MotionSensor.avatar` + bound entity on): three-view appends synthetic targets (`key: 'ai_<id>'`, `TargetWorld.ai`); a renderer-side controller owns a virtual raw position walking A*-verified goals in the sensor's region (WANDER → IDLE 4–15 s → ENGAGED 20–45 s when sit/activity/lie captures it). Everything downstream (nav, dwell, activities, bubbles, despawn fade) treats it as radar truth.
