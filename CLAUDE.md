...
- **Shared rule engine** `src/value-rules.ts` (PURE, three-free, imports nothing —
  ...
  binary_sensor short labels → enum Title Case → numeric+unit heuristic; °C→°F when
  imperial; prefix/suffix; relativeTime for ISO timestamps). `formatClock(mode, now,
