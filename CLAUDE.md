...
Settings ▸ Display. **`Layers2D.bgText`** (absent = ON, label "Background text", in the shared `LAYER_DEFS` + `bgText: false` in `SIMPLE_LAYERS`): three-view folds the flag into `_keyBgText` and passes an EMPTY entries list when off — rigs dispose through the normal rebuild path, `_bgTextPhase` survives so re-enabling resumes mid-course. Tests: `bgtext-multi-test.html` (`BGTEXTMULTI PASS 146/146`).
...
`bgtext-multi-test.html` (`BGTEXTMULTI PASS 146/146`).
...
- **`grass`**: a flat text decal on the ground at **y≈6** (between ground patches y=4 + blob
  ...
  advance SNAPS, so a `_keyBgText` rebuild re-acquires the camera pose instantly. Both
  placements (margin strip AND grassAreaId) pivot about their rect centre; `scale` overhang at
  some yaws accepted. `_bgGrassPlacement` picks the WIDEST of the four margin strips around the
...
- Test pages `terrain-test.html` (86/86), `fence-gate-test.html` (37/37).
