...
(inventing a creditor would be worse). The ISS is deliberately unaffected by an unconfigured
aircraft proxy (separate CORS-open feed, own cadence). Then `local` (the user's receiver `aircraft.json` URL —
...
OSM line) whenever cloud + enabled + data. Tests: `flights-test.html` (`FLIGHTS PASS 989/989 (§30 pure + §8z the rest_command proxy path end to end, real OpenSky + adsb.lol fixtures)` —
...
(`FLIGHTSUI 391/391` — settings round-trips, flight modal matrix, 2D hit routing; alert-center
67/67 stays green).
