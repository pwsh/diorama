...
Settings ▸ Display. **`Layers2D.bgText`** (absent = ON, label "Background text", in the shared `LAYER_DEFS` + `bgText: false` in `SIMPLE_LAYERS`): three-view folds the flag into `_keyBgText` and passes an EMPTY entries list when off — rigs dispose through the normal rebuild path, `_bgTextPhase` survives so re-enabling resumes mid-course. Tests: `bgtext-multi-test.html` (`BGTEXTMULTI PASS 309/309`).
...
`bgtext-multi-test.html` (`BGTEXTMULTI PASS 309/309`).
