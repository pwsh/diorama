...
+ validate.mjs checks 9–16, 503 total — 6 global + 15 per floor + 1 multi-floor
stair-link)**: doorway clearance (no nav-blocking piece in a
...
`wallCutsForSegment`; mountable/wall-plane/stairs exempt — **the old blanket
`elevation ≥ 300` exemption is GONE (2026-07-30): elevated pieces are excused only
when `depth ≤ WALL_MOUNT_MAX_DEPTH` 300, i.e. genuinely plate-like — a stacked
dryer at 990 mm was hiding in a wall behind it; rugs capped at `RUG_OVERLAP_TOL` =
wall thickness**), **check 13 (2026-07-30): bodied LIGHT fixtures vs walls**
(fireplaces were invisible to check 10 — one plan had a firebox opening INTO a
wall; `lib.mjs`'s settle pass now also runs the REAL `snapFireplaceToWall`/
`snapFloodlightToWall`/`snapExhaustToWall` via `settleLights()`), **nav
reachability** (a faithful `_buildNav` replica — **GUARDED by the differential
`test-pages/nav-parity-test.html` (`NAVPARITY PASS 70/70`, 8 fixtures incl. the window-vs-door twin (a window-only gap must NOT merge regions), cell-for-cell +
...
**That settle pass only ever corrected pieces carrying `def.seat`, so the SAME
"+Y = front" defect survived untouched in everything else until 2026-08-09**,
when a full library audit corrected **~400 furniture entries across all 18
plans** (beds onto canonical kinds/footprints with headboards on the wall,
~250 wall-backers turned to face the room, seats aimed at their conversation
focus, body interpenetrations resolved) — with ZERO non-furniture diffs in any
generated store. **Every one of those defects passed a green 425/425 run**,
which is why checks 14–16 exist:
- **Check 14 `bodyOverlaps`** (`BODY_OVERLAP_TOL` 250 mm) — checks 9–13 only
  ever measured a piece against WALLS or DOORS, so 305–650 mm furniture merges
  (a dishwasher inside a fridge, a coffee table inside an ottoman, a nightstand
  half-buried in a king bed) passed everything. Same-elevation
  (`STACK_ELEVATION_MM` 100) floor-standing non-mountable solid bodies only.
  Exemptions, each load-bearing: the **fitted casework run** (both kinds in
  `FITTED_RUN_KINDS` AND one a `WORKTOP_ANCHOR_KINDS` counter/island/cabinet —
  so counter×fridge lapping 512 mm lengthwise is the galley idiom while
  dishwasher×fridge is still a defect), **L/U sectionals** (the rotated bbox is
  not the body — the re-entrant corner is where the coffee table belongs), the
  `tv|tv_stand` stack, **planting** pairs (a bush at a tree's base), and
  **seat × host** (check 12 owns that at its own 200 mm tol). Threshold is
  DATA-DERIVED: every legitimate non-exempt lap in the library sits ≤ 205 mm,
  every confirmed defect ≥ 252 mm.
- **Check 15 `reversedBackers`** (`BACKER_FRONT_GATE` 450 / `BACKER_BACK_MIN`
  600) — THE systemic defect: a piece whose functional front is jammed against
  a wall with the whole room behind it. Everything downstream reads that front
  (2D chevron, humanoid facing, standing activity anchors, cabinet doors,
  screens, seat openings), so a reversed piece puts the avatar inside the wall.
  **A 150 mm front gate UNDER-REPORTS badly** (207 found vs 232 real) — 450 mm
  is the person-width band below which nobody can stand at the piece. Three
  false-positive classes are designed out: 5 samples across the face must ALL
  be blocked (a single centre ray reports a counter whose midpoint lines up
  with a partition END), the wall must be within 30° of PARALLEL (a
  perpendicular stub is a corner, not a backing wall), and `usedFromBothSides`
  exempts a 4-sided table (an `eat_at_table`/`work_at_desk` host with a
  captured seat BEHIND it). `frontArrow === false` kinds (island/ottoman/stool/
  plants) and the shapeless `block` fallback opt out. **SIGN CONVENTION — an
  agent inverted this once and only caught it by probing**: `frontVector(r) =
  (−sin r, −cos r)` ⇒ r 0 faces −Y, **r 90 faces −X (WEST), r 270 faces +X
  (EAST)**.
- **Check 16 `openingWallBurials`** — check 10 measures against
  `solidWallRuns`, where `wallCutsForSegment` has EXCISED every door and window
  (deliberately, so a window seat or a radiator under a bay reads clean). The
  side effect is a blind spot **the width of every opening**: a bookshelf shoved
  into the wall behind a window measures exactly 0 mm and check 10 says nothing.
  This re-measures the same footprints against the UNEXCISED centerlines with
  the same `wallCollidable` exemptions and tolerance, so the only pieces it can
  add are ones sitting in an opening (kitchen sinks and ranges centred ON
  exterior walls under their windows — 9 real cases). NB SAT depth saturates at
  the 100 mm wall thickness. `settleWalls` deliberately keeps using the EXCISED
  runs — an author may legitimately want a piece in a bay, so the check REPORTS
  rather than the settle pass silently moving it.
Negative control for all three: **19 / 232 / 9 findings on the pre-fix library,
0 / 0 / 0 on the fixed one.** `lib.mjs`'s `settleSeats` and `physical.mjs`'s
`seatAlignment` also pass the floor's walls as `seatBelongsToTable`'s trailing
wall guard and skip `NON_TABLE_SEAT_KINDS` — byte-identical on the corrected
library, and on the PRE-fix library it un-captures exactly the two cross-
partition toilets the audit had to hand-nudge around.
...
- **Fire flicker is REGISTERED, not baked (2026-08-10, user-reported perf — this REPLACES the old "an ON fireplace forces `updateLightsSwitches` every frame" exception).** A lit hearth / fire pit / heat lamp and a `flash`-flagged logic light used to compute their flicker INSIDE the builder, so three-view randomized `_keyLights` and rebuilt the ENTIRE light group every frame — 3.1–3.8 ms against an 11–13 ms render on a 200-piece / 24-light plan, i.e. **lighting a fire cost ~22 % of the frame rate** (worse on a busy plan — the reporting user measured 44 %) — and it is what made the stale-`matrixWorld` pick hazard PERMANENT rather than a rare race (see "3D fixture click"). The channels now register at build into `_lightFlickers` (**ONE entry per FIXTURE, never per channel** — a hearth's pool, embers, flames and point light must flicker in UNISON, exactly as the single shared `flickerMul` made them; the `fire` case also carries the warm-colour scale `cs`, the old `f1`) and `_fireFlames` (flame + hot-core cones: built at BASE height and posed per frame by `_applyFireFlame` writing `scale.y` + position + roll — a cone is centred on its origin, so seating the origin at `baseY + baseH·s/2` is arithmetically identical to re-extruding it). `_advanceLightFlickers()` runs from `_animate` beside `_advanceClimateGlows` (zero per-frame allocation, measured 0.001 ms) **AND once at the tail of `updateLightsSwitches`** so a rebuild never shows a neutral-brightness pop frame. Reset lives at the TOP of `updateLightsSwitches` BEFORE `_clearGroup` (the closures hold materials about to be disposed) plus `clearTransientGroups`/`destroy`. **`Math.random` in an ADVANCE is fine** — the house rule bans it in a BUILDER that reruns under a dirty key, and this refactor is what makes that rule hold here. three-view's force is GONE; `_keyLights` gained a `_flash` term (a flash starting/stopping changes which channels exist, and nothing else in the key is guaranteed to move with it). Envelope pinned before/after — every driven channel's min/max/mean matches its closed form, means within 1.8 % over 150 draws (firepit-test §6, `FIREPIT PASS 110/110`; G17 = 40 ticks with a lit pit → 0 rebuilds, G18 = putting it out → exactly 1).
- Still legitimately force-rebuilt per frame: **alarming safety sensors + active alert beacons** — so `_syncPickMatrices` stays load-bearing and firepit-test §F stays a live pin (it calls `updateLightsSwitches` directly, and its generality case is an alarming detector).
If you add a renderer input (new prop, new entity dependency), **add it to the corresponding key** or the scene won't update.

**Particle-cloud rebuild continuity (2026-08-01 audit fix — rebuilds must be visually seamless)**: every recycling particle system that used `Math.random` at BUILD time (fountain plume, shower spray, HVAC vent puffs + appliance vents, sprinkler droplet fans) re-randomized visibly on each dirty-key rebuild. One shared mechanism in three-renderer.ts: `ParticleCloudState` + `_cloudState`/`_rndFrom` (mulberry32 seeded off `_hashStr(fixtureId)` — deterministic, mutates a passed record, no closures in recycle paths) + `_pruneCloudState`. Per-fixture state maps (`_fountainState`/`_showerState`/`_ventState`/`_applianceVentState` (LOCAL coords — a piece move keeps continuity)/`_sprinklerState` incl. the persisted rotor `st.sweep` phase) hold the live `pos`/`vel`/`life` buffers, adopted **by reference** at rebuild when the signature (count + geometry params) matches — continuity is EXACT (delta 0), advance functions byte-unchanged, and a genuine param change re-seeds deterministically. Pruned at the owning update's end, cleared in `clearTransientGroups`/`destroy`. Same idiom for water-texture drift: `_waterPhase` (ground areas/yardFill, keyed by owner id via `tex.userData.waterKey`) + `_poolWaterPhase` (separate map — different dirty key) resume the accumulated shimmer offset on the fresh clone instead of snapping to 0. NEW particle systems must follow this (never bare `Math.random` in a builder that reruns under a dirty key). `_keyPool` no longer folds `waterTempEntity` (verified not consumed by the 3D build; the temp chip is 2D + the id stays config-path for the sidebar). Pins: yardlife 59/59, thermostat 73/73, bath-water 69/69, path-pool 62/62.

### 3D scene appearance
`Store.scene3d` (`Scene3D` in types.ts): lighting `preset` (`night` default / `day` / `dusk`), `floorColor`, `floorTex` (`none|wood|tile|concrete` — procedural canvas textures, cached in `_texCache`), `wallColor`. Applied in `updateFloor(f, scene3d)` + `applyScenePreset`, AND (2026-07-29) in the 2D `drawFloor` fill+pattern — floor look is no longer 3D-only. Sidebar "3D Scene" section edits it.
...
- **Glass house & wall cutaway** (`Scene3D.glassHouse` / `Scene3D.wallCutaway`, ride the whole-object scene3d persistence): `updateGhostFloors(floors, currentId, scene3d, customObjects, layers, elevMm?)` stacks every non-active floor as a translucent shell (slab 0.30 / walls 0.10 / furniture boxes 0.18) into `_ghostGroup` under the `_keyGhost` dirty key — **story offsets are ELEVATION-driven**: `floorElevationMm(ghost) − floorElevationMm(active)` via the trailing `elevMm` map three-view builds from the FULL `store.floors` (absent map → the legacy `(i − curIdx)·STORY_H` stack, stale-chunk safe and identical under auto elevations); `renderOrder` stays index-based (transparent sort, not height). **Ghost content maps through the ACTIVE floor's world frame** (`asx(wx) = activeFw/2 − wx`, `asz(wy) = wy − activeFd/2`; active dims resolved from the passed floors list, `_fw/_fd` fallback) so identical world coords land at identical scene X/Z across stories — cross-floor registration matches the 2D peek underlay + world-frame geo landmarks (the pre-2026-07-20 own-center mapping misregistered floors with DIFFERENT rect dims by half the dim difference; equal dims masked it). The ghost's OWN w/d only SIZES its slab (rect 0..w×0..d positioned in the shared frame). The transit puppet already used `_w` — one frame now. Test `ghost-align-test.html` (`GHOSTALIGN PASS 22/22`). **Ghost compositing**: every ghost mesh gets `renderOrder = storyIdx − activeIdx` (below-stories draw BEFORE the active transparent pass, above after — pins the transparent sort against the off-origin slab centers; ghost materials were already `depthWrite:false`). **Loop-slab shape-y sign**: a ShapeGeometry rotated −π/2 maps shape (sx, sy) → scene (sx, 0, −sy), so the ghost loop-slab uses `sy = −asz(wy)` = `afd/2 − wy` (same negation the active loop builder documents) — feeding `asz` directly Z-MIRRORED every loop-clipped ghost slab (walls were right, slab mirrored; caught only by an off-center-loop fixture, ghost-align test 4). **Glass-house see-through** (`glass-see-test.html`, `GLASSSEE PASS 26/26`): under glassHouse the dark stairwell/void **void plane** builds `transparent 0.18, depthWrite:false` (opaque otherwise — wells must read as pits); `updateGroundAreas`/`updatePools` take a `glassHouse?` param (stale-chunk-safe default) — yardFill/terrace patches/skirts build `transparent 0.45, depthWrite:true` (the active-slab idiom), water/pool surfaces `min(existing, 0.45)`; no new dirty-key inputs (the toggle rides configRev). Ghost **furniture boxes obey the same layer gates as the active floor** — appliance-category pieces + bins ride `layers.appliances`, everything else `layers.furniture` (both flags folded into `_keyGhost`); ghost slabs + walls always draw (there is no Walls layer; walls always draw on the active floor too). Glass-house mode also drops the ACTIVE floor's wall build opacity 0.45 → 0.22 so rear walls don't cloud the see-through view. Wall cutaway defaults ON (`!== false`): solid wall pieces are tagged `userData.wallCut` + `baseOpacity` at build, collected into `_cutawayWalls`/`_cutawayGhostWalls`, and `_updateWallCutaway()` (per frame from `_animate`) fades foreground walls to 0.06 — predicate: camera on the wall's outward side AND wall roughly between camera and floor center, with an overhead guard so top views hide nothing. **Cutaway fade PERSISTS across `_keyFloor`/`_keyGhost` rebuilds (2026-07-31, user-reported "foreground walls flash opaque at irregular 1–12 s intervals" — CONFIRMED: any idle configRev bump (blanket `number.*`/`switch.*` slow-path, 50 W power buckets, `applianceJustFinished`, preset flips…) rebuilt walls at base opacity, then the dt-based lerp (`1 − exp(−dt/CUTAWAY_FADE_TAU)`, τ = `-(1/60)/ln(0.9)` ≈ 0.158 s — **frame-rate-INDEPENDENT since 2026-08-06**: the old fixed per-frame 0.1 factor made τ ≈0.08 s at 120 Hz — a contributor to the "flashing when moving the camera" report — and ~2 s at 4.5 fps, measured; the closed form is exact, needs no substepping (the ω·h gotcha belongs to the spring integrators, not this), dt clamped ≤0.1 s with a 60 fps fallback, a no-arg call still = one 60 fps frame, and BOTH directions share one k — the symmetry the single 0.1 factor always had. Measured on the reporting user's plan: mean orbit flicker 0.0228→0.0128 %, max 0.21→0.047 %; a motion-lengthened τ was measured and REJECTED — worse mean, 1.4 s settle) decayed back — a ~0.2–0.5 s pulse; stairs were the worst, reborn at 1.0 vs cutFloor 0.12)**: `_wallFade`/`_wallFadeGhost` (+ `Prev` pair) persist each eased opacity under a stable key (`w:<wallId>:<segIdx>:<meshIdx>` / `s:<furnId>:<matIdx>` / `g:<floorId>:<wallId>:<segIdx>`, on `userData.cutKey`+`cutGhost`); `_tagCutawayWall` births a carried material at `Math.min(carried, base)` (the clamp so a glass-house toggle lowering the build opacity can't birth brighter than its new base; `baseOpacity` stays the fade-target authority), `_updateWallCutaway` writes the eased value back every frame (BOTH directions continuous — ease-back-up resumes mid-climb), lifecycle = swap-and-restart per build (self-pruning, one generation) + cleared in `clearTransientGroups`/`destroy`. The foreground predicate itself never oscillates with a static camera (0 flips/150 frames, test-pinned). Test `cutaway-persist-test.html` (`CUTAWAY PASS 41/41` — T8 dt-invariance (0.5 s at 30 fps vs 120 Hz within 1e-6, negative-controlled: the per-frame ease gave a 0.20 spread) + T9 robustness (tab-resume clamp, NaN/negative dt)). NB the harness copy-order trap: `test-pages/assets/` is gitignored but populated — copy test pages FIRST, dist assets SECOND, or a stale renderer bundle silently clobbers the fresh one. **`test-pages/*.mod.js` are the SAME trap (2026-08-15)** — 33 of them are gitignored but present in the working tree, so a blanket `cp -r test-pages/* $HARNESS/` overwrites freshly built bundles with stale ones (observed as `CR.hitFlight is not a function`); the safe order is **page + fixtures → dist assets → build every `.mod.js` LAST**. **IndexedDB pages need a DIFFERENT RUNNER (2026-08-19)**: the standard `--dump-dom` + `--virtual-time-budget` recipe CANNOT run an IDB-backed page — `--dump-dom` fires at load and does not wait for a module's top-level await (capturing the page mid-run), and virtual time does not advance IndexedDB's own sequence, so `open()` never settles. Reproduced against the pre-existing `avatar-store-test.html` (same hang), so it is the HARNESS, not the page — `avatar-store-test.html` documents no run recipe, which is why this went unnoticed. Drive IDB pages over CDP instead: a PERSISTENT `--headless=new` Chrome with `--remote-debugging-port`, open the page with a **PUT** to `/json/new?<url>` (a GET is 405 on current Chrome), then poll the target's `title` in `/json/list` until it carries the PASS line. `mmwave-history-test.html`'s header carries the full recipe. Toggles: 🏠 button in the 3D bar + 3D Scene sidebar checkboxes.
- **Target pathfinding** (all renderer-internal): `_buildNav` (end of `updateFloor`) rasterizes a 150 mm nav grid — furniture footprints inflated by `PERSON_R = 170` block (rugs/stairs/BEDS/`elevation ≥ 300` don't — beds are occupiable), solid wall runs block, DOOR openings stay walkable — **WINDOWS BLOCK like solid wall since 2026-08-05 (user-reported "avatars break through windows"): `wallCutsForSegment` gained an ADDITIVE `navSolids` field (complement of DOOR cuts only; returns `solids` BY IDENTITY when a segment has no windows — window-free plans byte-identical; `solids` stays the visual truth for 2D strokes + 3D extrusion; `WallOpeningCut.kind` was already the discriminant) consumed by the rasterizer, `_nav.wallSolids`, and the physical.mjs replica in lockstep** — then flood-fills free cells into connectivity **regions** (`_nav.region`, sizes in `_nav.regionSize`). All snap/retarget searches are region-aware (`_nearestFreeCellInRegion`) so avatars never land on the far side of a wall; the base `_nearestFreeCell` is additionally **region-size-aware** — after the first ring `r0` with a free candidate it scans through `r0+4` rings and returns the candidate in the LARGEST region (tie-break nearest), so spawns / retargets / `_regionOfWorld` snaps prefer the real open room over a tiny sliver channel (e.g. the strip between a sofa's inflated footprint and the wall behind it) while never failing when only a sliver exists; a goal unreachable for >3 s (`h.stuckT`) fast-fades the rig and respawns it — **in the GOAL's region for radar/BLE rigs only (ground truth); SYNTHETIC rigs (ai/demo/roam) respawn in their OWN region since 2026-08-06** (a walker must never teleport across a boundary it couldn't cross; NB `stuckT` is structurally blind whenever a door merges indoors+outdoors into one region — same-region goals always "reach"). Movement is a **carrot-chaser**: the carrot walks the A*-string-pulled polyline by arc-length (the carrot IS the pathfinding walker); `navX/navZ` chase it with a critically damped spring (`nvx/nvz`, ω = 9, substepped ω·h ≤ 0.36 — same divergence gotcha as stepLerp). **Two speeds**: `navSpeed` (nav deltas) drives gait/facing; `rawSpeed` (raw radar deltas) drives sit/activity/bubble triggers — don't cross them. Anchored rigs (`sit/act/lie > 0.3`) bypass nav; pairwise separation holds figures ~380 mm apart (skips lying rigs). 2D dots stay raw radar truth. **Real-position markers (2026-08-07, user-requested)**: `Sensor.showRealPositions?` (item-level, absent = off; mmWave editor checkbox) shows each active radar target's RAW UN-SMOOTHED report — `LerpSlot.tx/ty` (the spring's GOAL, written straight from `stateMm` per push; `cx/cy` is the EASED position the 2D dot + nav goal read) via `localToWorld` — as a 3D emissive ball (r 90 mm at y 1000, target tint, outlineSkip, no blob shadow; `_realPosMarkers` synced per frame in `updateTargets` off optional `TargetWorld.showReal/rawX/rawY`, in `_targetGroup` so the Avatars layer/kiosk/floor-switch teardown come free) + a 2D hollow ring after the smoothed dot. Deliberately SNAPS at push cadence (never eased — the honesty IS the feature) and its lifecycle is INDEPENDENT of the rig's (report gone = marker gone at once while the rig runs its despawn fade). Synthetic rigs (ai/demo/roam/ble/cam) never get one. Test `real-pos-test.html` (`REALPOS PASS 41/41`; NB its bundle is `demoav`-style lit+three aliased; demo-avatar-test's bundle must be named `demoav.mod.js` — the page imports that exact name). **Radar room confinement (2026-08-07, user-reported "sensor detects them further than the wall, avatar generated on the other side")**: `Sensor.confineToRoom?` (item-level, absent = off byte-identical; mmWave editor "Keep avatars in this room" + a dim hint when the sensor isn't inside any loop = no-op) clamps a radar target's DISPLAY into the wall loop containing the SENSOR: pure `clampPointToLoop(loop, x, y, insetMm = 60)` (geometry.ts — inside = passthrough; outside = nearest boundary point nudged inward, inward DERIVED via pointInPolygon-verified candidates (push-back → centroid-aim → centroid), never returns a point further out); three-view stamps optional `TargetWorld.confine/srcX/srcY` (sensor world pos); renderer memoizes loops in `_confineLoops` keyed by ROUNDED sensor pos (all 3 slots share; dropped at both `_wallLoops` write sites so wall edits can't stale it) and clamps the raw-truth inputs BEFORE nav; **`_confinedCell` is region-first-room-second** (a clamped point rests ON the wall band where `_regionOfWorld` can answer the FAR side — the naive path silently undid the clamp through the fallback; verify-in-loop then retry room-only); **the stuck detector reads the confined GOAL CELL's region, not the raw point's** (else a confined rig respawn-flickers every 3 s forever; a genuinely stranded rig still mismatches once, respawns, settles). 2D dot clamps via the `floorLoops()` cache; the `showRealPositions` raw ring stays UNCLAMPED (the pairing IS the design: clamped avatar + honest marker). Fusion candidates, zone/halo occupancy deliberately unclamped. Test `confine-test.html` (`CONFINE PASS 61/61` — no-door two-region fixture, 300-frame no-escape, live toggle, per-sensor independence. **`--run-all-compositor-stages-before-draw` IS LOAD-BEARING for this page (2026-08-19): omitting it yields a deterministic `CONFINE FAIL 58/61` (assertions 5c/5d/5e — 5c off by 1 mm), which is a HARNESS artifact and NOT a defect.** An agent reported that 58/61 as a pre-existing HEAD failure and it was briefly recorded here as one; re-running the page's OWN documented recipe gives 61/61, stable over 3 runs, and dropping just that flag reproduces the 58/61 exactly. Same class as the `--use-gl` mistake: when a suite 'fails', re-run its documented flags verbatim before believing it.)
...
- **Lying in bed**: settled occupant (>2 s) within the bed's lane capacity (`bedLieCapacity(w)` = `max(1, floor(w/BED_LANE_WIDTH_MM))`, 700 mm — geometry.ts, the ONE home for the divisor) → `h.lie` blend: root pitch +π/2 (face up), yaw toward the headboard, y = mattress top, x offset into the assigned lane; enters from the FOOT end (never through the headboard); plumbob hidden, bubble repinned in world space. Occupants beyond capacity stand (lane −1, no stacking). Capacity: twin 990 → 1, full 1370 → 1, **queen 1524 → 2, king 1930 → 2**. **TWO AVATARS LIE ONE PER SIDE ON WIDE BEDS (2026-08-20, user-reported "avatars should render one either side instead of just in the middle") — the lane machinery ALREADY existed and was already correct in shape; it was unreachable for TWO INDEPENDENT reasons, so fixing either alone does nothing.** (1) **The gate**: `coversWouldHide` forced `wantLie` false whenever a bed had ≥2 occupants AND `sharedCovers`, which resolved as `fu.sharedBedCovers !== false` = **default ON** — so with two occupants NEITHER avatar ever lay down; they STOOD in the footprint until the blanket lump engaged (>2 s) and hid them both, and side-by-side was reachable only by manually unchecking the box on that piece. FIX: `resolveBedSharedCovers(fu)` (geometry.ts) = explicit value WINS, else `bedSharedCoversDefault` = `capacity < 2` — blanket for twin/full (one lane: the lump is the only two-in-bed read), lanes for queen/king. **Resolved ONCE at the `_beds` record** so `coversWouldHide` and `_updateBedCovers` inherit it and cannot diverge — never add a second capacity test at either site. (2) **The spacing**: `bedLaneOffsetMm` divides by `used`, NOT `used + 1`. The old divisor put a pair at ±w/6 (queen ±254) — and that was not merely "reads as the middle": MEASURED off the built rigs, two 559.2 mm-wide bodies **INTERPENETRATED by 51.2 mm** on a queen. At ±w/4 (queen ±381, king ±482.5) the bodies gap 202.8 / 405.8 mm with 101 / 203 mm of mattress spare each side. Explicit `sharedBedCovers: true` restores the blanket + hidden rigs exactly; the sidebar checkbox renders the RESOLVED default (a queen shows unchecked). Test `bed-lanes-test.html` (`BEDLANES PASS 74/74`, `ok(name, cond, neg)` discipline — an assertion without a negative control is itself a failure; mutation-tested: divisor→`used+1` fails 16, covers-always-ON fails 47, both reverted fails 49). **NOTED, NOT DONE**: a full/double (1370) sleeps two in reality but is single-lane at 700; a ~680 divisor would flip full→2 without touching twin (990→1), but it also moves the boundary for any user-resized piece — a separate, testable change.
...
- **AI avatars & demo mode + respawn re-roll**: `MotionSensor.demo` (item-level, persisted; sidebar "Demo avatar" checkbox in the motion editor) makes three-view append the synthetic AI target **regardless of entity binding/state** — `m.demo || (m.avatar && bound entity on)`. A demo rig is pure display (rendered in kiosk/view modes too) and uses the sensor's avatar pool + home-room confinement exactly like a normal AI avatar. **mmWave sensors have the same demo mode since 2026-08-05** (`Sensor.demo?`, sidebar "Demo avatar" checkbox in the mmWave editor — its own three-view loop AFTER the radar loop, key `ais_<sensorId>` distinct from radar `<sensorId>_<i>` and motion `ai_<motionId>`; home-room-confined via the sensor's position, uses `Sensor.avatarKinds`/`plumbobColor`; a BOUND sensor shows real radar rigs AND the demo rig). **Synthetic avatars (AI/demo/roamer) are 3D-ONLY** — 2D `drawTargets` reads radar/BLE/cam sources only; the AI controller state lives inside the renderer chunk, so the plan view shows nothing for them (a repeated user-confusion source — investigated 2026-08-05, NOT a regression: `MotionSensor.demo` was verified working end-to-end, and note `avatar: true` on an UNBOUND sensor renders nothing BY DESIGN — demo is the unbound switch). Test `demo-avatar-test.html` (`DEMOAVATAR PASS 23/23`; its bundle needs the lit + THREE aliases — the header recipe has them). **Re-roll**: when a rig is created FRESH (no existing `Humanoid` for the key) and its avatar spec is a POOL (`avatars` array of ≥2, or `'random'`/unknown), `resolveAvatar(..., Math.random)` picks RANDOMLY instead of the stable djb2 hash, so a respawn looks different. `avatarFromPool()` gates this AND exempts pool rigs from the kind-mismatch rebuild (the per-frame stable pick must not fight the rolled kind); a recolor carries the rolled kind over (`forcedKind`). Explicit single kinds and identified people (`person.avatarKind`/pet) never re-roll. Mid-fade re-acquire keeps the same rig unchanged.
...
- **Camera pivot / free movement** (`Scene3D.pivotLocked?` **absent = true** + `Scene3D.freeMovement?` **absent = false** — two INDEPENDENT booleans; `Scene3D.cameraPivot` is DEPRECATED, still READ for back-compat, never written again; ✋ free-movement + 📌 pivot-lock 3D-bar buttons + two checkboxes in Settings ▸ Display Camera; rides scene3d whole-object + the card `scene3dOverride` merge, NOT in the card editor/validator). Resolution is the pure `resolvePivotMode(sc3)` (geometry.ts, types-only): either new field present ⇒ use them (defaults above), else legacy `cameraPivot === 'free'` ⇒ `{locked:false, free:true}`, else `{locked:true, free:false}`. `setCameraPivot(locked, free, cx, cz)` (self-guarding per tick like setFov; the CENTRE refreshes unconditionally — it moves with the floor rect / glass-house union) drives the matrix:
  ...
  Pivot centre = current floor centre (scene (0,0)), or `floorsUnionCenter(enabledFloors)` (pure, geometry.ts — origin-anchored rects ⇒ `max(w)/2, max(d)/2`) mapped through the ACTIVE floor's `_w()` frame under glassHouse. **Precedence**: `_updateCameraPivot` early-returns while autoFollow OR cinematicOrbit (they own the target; pan stays disabled) — the custom rotate deliberately still WORKS during them and pauses them, exactly as stock rotate does. Presets/`cam=` poses set their own target and then ease home (accepted, mode-consistent).
  **Pointer-ray wheel zoom (`Scene3D.zoomToCursor?`, absent = false = today byte-identical; Settings ▸ Display ▸ Camera "Zoom toward the pointer"; 2026-08-19, user-reported "zoom speed slows down and becomes difficult to zoom into the property… front zooms quickly, rear zooms slowly")**: OrbitControls' dolly is multiplicative in the distance to `controls.target` — one notch moves EXACTLY `0.05 × radius` (measured constant to 5 dp over 60 notches, `1 − 0.95^1`) — and the locked pivot WELDS that target to the plan centre, so the camera can only ever converge on one point. On the reporting user's 32 × 52.2 m plot that is the whole bug: a rear-yard subject's apparent size grew 3.68 %/notch at radius 45 000, 2.35 % at 16 132 and 0.93 % at 4 475 (decaying to nothing) while a subject in FRONT of the pivot held 7.2–8.4 %/notch — the 8× asymmetry the user felt — and the camera bottomed out 19.8 m from the rear yard, magnifying it only 3.02× against 21.71× for a subject at the pivot. **OrbitControls' OWN `zoomToCursor` was implemented, measured and REJECTED** (do not re-try it): it dollies along the pointer ray but still drives the radius to `minDistance`, so a gesture's TOTAL travel is capped at `radius − minDistance` ≈ 44 m — less than this plot is deep — and it measured strictly WORSE for the rear yard (best approach 20.8 m → 28.0 m, notch-45 growth 0.93 % → 0.37 %) because it spends that fixed budget on a ray that only asymptotically aligns. The shipped mechanism is a renderer-owned `wheel` handler (`_installCursorZoom`/`_applyCursorZoom`, the `_installPivotRotate` precedent; zero per-notch allocation via module `_cz*` scratch) that measures the step against **what is under the pointer** — the ray's intersection with the floor slab plane (scene y = 0; a plan is flat and everything of interest stands on it, so a plane hit is exact, allocation-free and cannot be fooled by a wall or the sky dome; a ray that does not descend falls back to the current target = today's plain dolly). Camera AND target each move `1 − scale` of the way to that point, so the camera→target offset scales by exactly `scale` (a cursor ON the target reduces to today's dolly bit-for-bit) and the distance to the pointed-at thing shrinks a constant 5.26 %/notch **for every subject** — measured after: magnification 21.7–21.8× and rate 5.26–5.28 % at EVERY stage for all six probe subjects (front property line → rear property line), vs 2.58×–21.71× and 0.74–5.26 % before. The budget renews because the step is a fraction of a real scene distance, so the camera crosses the whole property (traced: scene z −41 906 → +15 965, ~57 m of travel) while the subject stays pinned at its cursor NDC. **Reconciliation with the lock (load-bearing — the two MUST travel together)**: the dolly moves the target and `_updateCameraPivot`'s weld would drag it — and, because that ease translates the CAMERA by the identical delta, the whole view — straight back, i.e. a rubber band. So under `zoomToCursor` the locked-pivot enforcement relaxes from a POINT weld to a **PLAN-RECT CONTAINMENT**: inside the floor's own footprint the target is left completely alone; outside it eases back to the nearest in-bounds point by the identical mechanism. The documented guarantee — *the orbit pivot can never drift OFF the plan* — is what actually holds either way, and the hit point is ALSO clamped into the rect at the source, so the target is a convex blend of two in-rect points and can never leave. Both clamps are gated on EXACTLY `locked && !free` (under free movement the user is deliberately allowed off the plan — out among the neighborhood buildings — and clamping would fight the pan). `setCameraPivot` gained a TRAILING OPTIONAL `opts {halfW, halfD, zoomToCursor}` (stale-caller/stale-chunk safe — a 4-arg call is byte-identical today, and half-extents 0 = unknown SKIPS the containment so an old call site can never silently loosen the lock); three-view passes `pivotWorld` as the half-extents, which is exact rather than a coincidence — centre and half-extent are the same number for any rect anchored at the origin, in BOTH the active-floor and `floorsUnionCenter` glass-house branches. Registered CAPTURE-phase on `window` so it strictly precedes OrbitControls' own canvas `wheel` listener, which it swallows with `stopPropagation` — that listener is also what dispatches OC's 'start'/'end' for a wheel, so the two things those drive (the auto-follow pause + `cameraGestures()`, the boot-pose "user posed it by hand" latch, and the Sims-cam 45° azimuth snap) are reproduced explicitly. TOUCH PINCH deliberately still goes through OrbitControls unchanged. Zooming OUT is a plain reverse dolly (pulling the pivot AWAY from what you pointed at is not what anyone means by zooming out). Deliberate consequence: with the flag on, a preset / saved view / `cam=` pose that framed an off-centre point KEEPS its framing instead of sliding to the plan centre. NOT in the card editor/validator (consistent with `pivotLocked`/`freeMovement`), though it rides the card `scene3dOverride` merge like the rest of scene3d. Test `camera-test.html` §15 (`CAMERA PASS 88/88`, was 66/66 — independently re-run by the orchestrator) — incl. a harness self-check that `right = cross(forward, worldUp)`, because getting that handedness backwards mirrors the vertical axis and silently aims every simulated "click" at the opposite end of the plot, which is exactly how this was first mis-measured into a wrong conclusion.
...
the ISS rendered in the 3D sky. **Sources**: `opensky` (**the DEFAULT** — opensky-network.org
`/states/all`), `adsblol` (api.adsb.lol v2), `cloud` (airplanes.live), `local`, `entity`.
**THE BROWSER-DIRECT TRANSPORT IS DEAD AS A CONCEPT, not merely as a provider** (re-measured
2026-08-15 with curl AND a real headless browser from a foreign origin — the earlier note here
...
CORS permission; a browser always does**, which is the whole distinction (a user reporting "but
this URL returns JSON" is right and is not contradicting this). There is no longer ANY keyless
CORS-open ADS-B API. OpenSky's terms cover personal, non-commercial use — which is what this is.
So `opensky`/`adsblol` are fetched **SERVER-SIDE by Home Assistant** through a user-defined
`rest_command` invoked with `return_response: true` — the `weather.get_forecasts` /
`calendar.get_events` mechanism, generalized as `HaApi.callServiceWithResponse(domain, service,
data, target?)` (both clients, inert in `LocalApi`; the two pre-existing call sites were
refactored onto it behaviour-identically). `FlightsConfig.proxyCommand` holds the service name and
**ABSENT is the "not configured" sentinel** → `flightsStatus: 'needs-proxy'`, which calls NOTHING
(no service call, no fetch) rather than hammering a missing service; `_pollFlights` carries the
same guard so an explicit poll can't downgrade it to a generic `error`. Settings renders the exact
YAML to paste, generated by the pure `flightProxyYaml` **from the SAME helper that builds the
service data the poll sends**, so the two can never disagree; the URL is TEMPLATED, not baked, so
a radius change never means editing YAML. **Absent/unknown `source` resolves to `'opensky'`**
(`resolveFlightSource`/`FLIGHTS_DEFAULT_SOURCE`) but a stored EXPLICIT source is NEVER rewritten —
a migrating `cloud` user keeps `cloud` and is told in the UI that airplanes.live 403s. **Poll
default is source-aware** (`flightDefaultPollSeconds`): 60 s for OpenSky (credits ~400/day
anonymous, ~4000 with an account, 1–4 per request — at 8 s that would be ~10 800/day), 8 s
otherwise; the 5–60 clamp is unchanged. `openSkyBoundingBox(lat, lon, radiusNm)` derives the
lat/lon BOX OpenSky needs (adsb.lol and airplanes.live take point+radius): latitude half-span =
the arc angle, longitude half-span = that ÷ `cos(lat)`, edges rounded **OUTWARD** at 5 dp, and
both degenerate cases (very high latitude, antimeridian straddle) **WIDEN to the full longitude
range rather than clipping** — over-fetching is free because the planner filters by true distance
afterwards, whereas clipping loses aircraft silently. `normalizeOpenSkyStates` maps the POSITIONAL
SI-unit state arrays onto readsb field names and delegates to `normalizeAircraftList`, so the
ground filter, the `C*`/`B3` non-aircraft drop and the never-throws discipline are inherited by
construction (m→ft, m/s→kt, m/s→ft/min; callsign is space-padded — trim it; OpenSky's numeric
emitter-category enum is the DO-260B table relabelled `2→A1 … 8→A7, 11→B3, 16–20→C1–C5`).
**CAPABILITY GAP**: `/states/all` carries no registry enrichment and no dbFlags, so
`reg`/`typeCode`/`typeDesc`/`operator` are null and `military`/`interesting`/`pia`/`ladd` false —
military skins, privacy dimming and type-driven archetypes DO NOT fire on OpenSky; callsign-derived
airline livery still does. adsb.lol reuses `normalizeAircraftList` UNCHANGED (verified against a
real 818-aircraft capture, not assumed). `normalizeFlightPayload(source, json)` is the ONE dispatch
point; `unwrapRestCommandPayload` peels `{context,response}`→`{status,content}` in any combination
and returns null on unparseable text. **Attribution follows the SOURCE** (`flightAttribution`):
"© The OpenSky Network" / "© adsb.lol" / "© airplanes.live"; `local`/`entity` credit nobody
(inventing a creditor would be worse); `demo` credits nobody either — nobody is owed credit for
invented data. The ISS is deliberately unaffected by an unconfigured
aircraft proxy (separate CORS-open feed, own cadence). Then `local` (the user's receiver
`aircraft.json` URL), `entity` (HA
rest-sensor proxy — attributes carry an aircraft array under `aircraft`/`ac`/`flights`; the bound
id is config-path in `_isSlowEntity`).
**`local` may ALSO route through the `rest_command` proxy (2026-08-15, user-requested)** — HA
fetches the LAN receiver server-side, which erases BOTH of that source's restrictions at once: no
CORS (a server-side fetch sends no `Origin` and triggers no preflight, so the tar1090 lighttpd
block in research §2.2 becomes unnecessary and the receiver is not touched at all) and no mixed
content (HA↔receiver is server-to-server, panel↔HA same-origin — an HTTPS panel can finally read a
plain-HTTP Pi). **OPTIONAL, never mandatory** — unlike opensky/adsblol, direct-fetch remains
legitimate for an HTTP panel that added the header, so `local` carries a suggested command name
(`FLIGHT_PROXY_DEFAULT_COMMAND.local = 'diorama_local_adsb'`) yet is deliberately ABSENT from
`flightSourceNeedsProxy`: an empty name means "keep fetching directly", never an error state.
`flightProxyVars` returns `{}` for local (no coordinates ⇒ no Jinja; the block is static) and
`flightProxyYaml` gained a TRAILING OPTIONAL `localUrl` (stale-caller idiom); an unset URL emits a
marked `# SET THE RECEIVER URL…` placeholder rather than a block that looks pasteable but fetches
nothing. Also better than the `entity` workaround it replaces: a rest SENSOR pushes the whole
aircraft list through HA's state machine + recorder as attributes, whereas `return_response`
...
(caller-supplied epoch, no `Math.random` — the dirty-key rule), flies closed circuits about the
observer, and **central-differences `gs`/`track`/`vertRate` from the analytic position**
(`DEMO_DIFF_S` 1) so the reported motion is the TRUE tangent of the drawn path — measured: track
exactly ±90° from position bearing, and dead-reckoning 8 s along `track` at `gs` lands within
0.055 nm of the generator's own next fix, so the existing `_advanceFlights` carries it with NO new
animation system. Orbit radii are FRACTIONS of the configured radius, so the shell-compression
spread and the deliberately out-of-range pair stay meaningful at any radius (5–100 nm); it routes
through the ordinary `_applyFlights`, so radius/altitude/cap filtering is identical and
test-pinned live. **The 16-row fleet doubles as deterministic visual coverage** — all 8
archetypes, all 5 speed bands, 6 real airline liveries, a towed banner, an F16 military skin, a
7700 emergency (beacon + `flight:emerg:` alert), a PIA (identity suppressed) and a LADD (identity
kept), a REACH callsign word as a skin NEGATIVE control, a regional colourless-livery veto, and
**two out-of-range aircraft at indices 6 and 11** so even a small `fleet` slice exercises the
radius filter. `DemoFlightsConfig` (`fleet`/`seed`/`emergency`/`lat`/`lon`, sanitized in
`setFlights`); `demo.lat/lon` is NEARLY A NO-OP BY CONSTRUCTION — every flight position is
origin-RELATIVE, so a synthetic origin yields an identical sky; it survives only because it moves
the **ISS**, the UI says exactly that, and the inputs are HIDDEN unless the origin actually
resolved to synthetic. The emergency row is ON by default with an opt-out: `alert-center.ts` hides
the bell entirely when offline, so it raises ZERO visible alert in the gh-pages demo, and in an HA
panel the user explicitly chose synthetic traffic — but the aircraft never leaves the feed, so the
alert STANDS rather than passing, which is what the checkbox is for. `src/demo-seed.ts` enables it
(`withDemoFlightsEnvelope`, fleet capped 8 — a visitor's device, not a stress test; a no-op on an
envelope that already authored `flights`) — done IN demo-seed, NOT by regenerating
`docs/floorplans/*.json` (generated artifacts). NB none of the 18 demo floorplans carries a
calibrated landmark or `weather.lat/lon`, which is exactly why the demo origin rung exists. **Pure data layer** `src/flights.ts` (ZERO-import,
...
idiom; only `enabled` + origin gate), poll clamp 5–60 s default 8, ISS on its own 10 s timer for
all sources (`iss !== false`); runtime `flightsNow`/`flightsAt`/`flightsRev` (bumps on ISS changes
too — ONE dirty-key input)/`issNow`/`flightsStatus`
(`off|no-origin|ok|error|needs-proxy|needs-ha`; 'error' only when nothing cached —
stale-tolerant) + public `flightsOrigin()` (geo-fit origin → `weather.lat/lon` → **synthetic demo
origin when `source === 'demo'`** → null — the observer chain, do not fork a second one; the demo
rung is LAST so a real calibrated home always wins).
**OFFLINE / gh-pages demo (2026-08-15 — the old note here claimed "offline does NOT block — the
cloud source works in the gh-pages demo"; that is FALSE twice over since airplanes.live began
403ing everyone and the default became a proxied source)**: the pure `flightSourceNeedsHa` (a
deliberate STRICT SUPERSET of `flightSourceNeedsProxy`, pinned) gates status `needs-ha` when
`HaApi.offline` — opensky/adsblol (the proxy can never answer) AND **`entity`** (an HA sensor
cannot exist without HA, and reporting a fetch `error` would be a lie — nothing was ever
fetched). Checked BEFORE `needsProxy` (offline, a missing service name is not the problem, so
offering YAML is not a fix) and deliberately NOT an early return, so **the ISS keeps flying** —
it is a separate CORS-open feed and was the only flight visual that survived offline before the
`demo` source existed. `_pollFlights` carries the same guard so an explicit poll can't downgrade
it to a generic `error`. Genuinely reachable offline: **`demo`** (zero network by construction),
and **`local` with a browser-reachable `localUrl`** — the one REAL aircraft source that serves a
Home-Assistant-less panel; a `local` fixture that ALSO names a proxy command **demotes to the
direct fetch** offline rather than refusing (HA definitionally cannot fetch, and the URL is right
there). `cloud` keeps its normal path (a real request that honestly 403s); `local` with no
`localUrl` is an incomplete config → `error`, not `needs-ha`. **Routine polls are LIVE-path — they must NEVER
...
OSM line) whenever cloud + enabled + data. Tests: `flights-test.html` (`FLIGHTS PASS 1279/1279 (§30 pure + §8z the rest_command proxy path end to end, real OpenSky + adsb.lol fixtures; §31/§36 the demo source; the offline needs-ha matrix; a generated 85-row local-receiver wide-catch fixture)` —
...
(`FLIGHTSUI 461/461` — settings round-trips, flight modal matrix, 2D hit routing; alert-center
67/67 stays green).
...
- **Moon disc**: a `THREE.Sprite` whose per-phase `CanvasTexture` (cached in `_moonTexCache`)
  ...
  the `night` preset. Unbound `moonEntity` → default full moon. **`WeatherConfig.moonStation?` (2026-08-01, user-requested "space station" checkbox — Settings ▸ Weather, "phases still apply" hint)**: true → `_paintStationDisc` paints a Death-Star-style battle station (grey hull + limb annulus toon shading, upper-right superlaser dish w/ rim + focus dot, equatorial trench band, deterministic panel lines — all clipped to the disc, no gradients, no Math.random) IN PLACE of the flat lit-disc fill, with the SAME unlit-semicircle + terminator overlay composing on top so it waxes/wanes like the moon; `_moonTexCache` keys `<phase>|station` (one map, wholesale destroy()-only lifecycle; a latent bug fixed here — `moonPhaseFraction` was fed the suffixed cache KEY, which would have defaulted unknown→full); BOTH moon paths covered by construction (one `_moonSprite`; decorative arc vs real `moonAltAz` differ only in position, test-pinned); the station flag rides `WeatherFxState.moonStation` → the skyBucket term; whole-object weather passthrough — no field-list change.
...
- **`_keyWeather`** gained `effPreset:moonPhase:skyBackdrop:observer(4dp lat/lon + θ):moonStation` (`skyBucket`) so the sky
  rebuilds on a preset flip / phase change / toggle / station toggle. Test pages: `weather-fx-test.html?c=sky`
  (69/69 — sun position recomputed, moon phase pixel-sampled, dome colors differ day/night/
...
`_makeGrassTextTexture` (TRANSPARENT since 2026-07-31 — the shared `_paintGroundText` letters-only painter; ink from `_bgGroundKindAt` = containing area kind → yardFill → grass, threaded via `updateBgTexts`'s optional trailing `ground` arg, stale-chunk → grass)
...
`mode:'chopper'` build VERBATIM). Each carries `len` (feeds the standoff `len/2 + 500 +
halfBannerLen`) + `idY`; signature paint per craft, tintable via `colorMain`/`colorDetail`
(mkBody/mkAccent); all `_mat({fog:false})`, no Math.random, no blob shadows, outline shells via
a fog-free clone of `_outlineMaterial` (news_chopper keeps its shipped shell-free look);
spacecraft carry STATIC emissive glow (no new per-frame system); rotorcraft ride
`BgRig.props[]`/`rotorY`. Absent/unknown → the classic toy plane, byte-identical. **The chopper
FLIGHT PROFILE keys off `BG_CRAFTS[craft].chopper`, never mode**: `news_chopper` → dir −1, alt
7500 AGL, radius ×0.6, bob 150, corner-hung banner + rigid tow wire (the "Chopper tow point"
contract byte-preserved; `BgRig.newsChopper` flag, `BgRig.mode` always `'banner'` for tow
craft); apache/airwolf/`heli` fly the ordinary orbit. **`mode:'chopper'` is DEPRECATED**:
`_migrateBgTexts` maps it → `{...e, mode:'banner', aircraft: e.aircraft ?? 'news_chopper'}`
...
Settings ▸ Display. **`Layers2D.bgText`** (absent = ON, label "Background text", in the shared `LAYER_DEFS` + `bgText: false` in `SIMPLE_LAYERS`): three-view folds the flag into `_keyBgText` and passes an EMPTY entries list when off — rigs dispose through the normal rebuild path, `_bgTextPhase` survives so re-enabling resumes mid-course. Tests: `bgtext-multi-test.html` (`BGTEXTMULTI PASS 373/373`).
...
- **`grass`**: a flat text decal on the ground at **y≈6** (between ground patches y=4 + blob
  shadows y=8) that **FACES THE CAMERA by default, with a per-entry static-rotation opt-out**
  (`BgTextEntry.faceCamera?` absent/true = follow; `false` + `rotationDeg?` = fixed — repo-wide
  screen-CW 0=+Y convention, ψ = π − deg·π/180 via the pure `bgGroundFixedYaw`; unclamped like
  `Light.rotation`; the static want feeds the SAME ease/shortest-arc machinery, and
  `_seedGrassYaw` applies it at build so a settings-edit rebuild SNAPS; a follow row emits
  neither field so its `_keyBgText` hash is byte-identical to pre-feature; `rotateFloorContent`
  deliberately skips bgTexts — store-level, not per-floor; modals: "Follow camera" checkbox +
  conditional "Rotation (°)"): the mesh keeps its own
  ...
  exposes it for the test). Letters-only transparent canvas (2026-07-31 — NO base rect ever; the surface beneath IS the backdrop; per-kind two-tone relief ink, blacktop chalk-INVERTED `#d3d8dd`/`#0e1113` — near-black-on-near-black failed without the box; grass-mode entries fold the pure `bgGroundInkKey(floor)` (geometry.ts — yardFill + per-area kind@100mm-bbox) into the `_keyBgText` entry hash so yard-paint edits re-ink WITHOUT configRev).
...
- **UI**: Settings ▸ Display "Background text" block — mode select, static message input (capped
  ...
  **all UI modes** (a display prop; no Layers2D entry — `mode` is the gate). Test:
  `test-pages/bgtext-test.html` (`BGTEXT PASS 30/30`; needs `value-rules.mod.js` via
...
- **Renderer** `_weatherGroup` + `updateWeather(fx)`: rebuilt only under three-view's `_keyWeather` — **NO configRev term (2026-08-01, user-reported "weather overlays redraw at entity changes"; the _keyBgText precedent)**: the composition is the pure exported `weatherRebuildKey(fx, floor, effPreset)` in `weather.ts` (three-view just calls it; testable without a component) = `floorId|w×d|condition|intensity·4|isDay|windBucket|w3Bucket|alertSeverity` — floor DIMS (spawn box/fog planes/storm-bank/puddle scatter derive from the bbox), `isDay` (lightning flash energy) and a 3° sun-elevation bucket (star ramp/sun-disc, sign kept for the horizon gate) were the terms configRev had been papering over; `weatherWindBucket(fx)` is SHARED with `_keyFlagpoles` so the two can't disagree; ground level/wall loops verified NOT consumed (puddles are floor-bbox+id-seeded, the group is never repositioned). Legit rebuilds still re-seed particles (accepted). Per-frame motion is `_advanceWeather(dt, nowS)` called every frame from `_animate` — **zero allocation after build**: it mutates each cloud's `position` buffer in place (`needsUpdate = true`), never reallocates.
...
- **Live drag dimensions** (edit-mode only, NOT layer-gated — they exist only while an
  interaction is in flight): the pure exported `liveDimChips(p): LiveDimChip[]`
  (`{x, y, text, dim?}`, WORLD-mm anchors — testable without a canvas) computes readout
  chips per drag/latch, and the dumb painter `drawLiveDims` (the LAST call in `drawAll`)
  paints them via `drawDimChip` (which gained a trailing `alpha = 1` param), clamped
  on-screen, never rotated, `fmtLen`-formatted (imperial-aware). Coverage: wall drawing
  (rubber band live + committed segments dimmed), `wallv` (all segments, adjoining live),
  `floorEdge`/`furnCorner`/`bgCorner` (`W × D` chip — NB furniture DOES have a canvas
  corner resize, `hitFurnitureCorner` → `furnCorner`, rotation-aware min 50 mm; bg corner
  resize `bgCorner`, shift = aspect lock), polygon draw latches incl. LD2450 `editZone`
  (rubber band), polygon vertex drags incl. LD2450 `vert` (adjoining edges; path = open
  polyline, no wrap). Skips: ruler (has its own live chips), `envResize` (unitless),
  every move/rotate drag. The old inline `wallv` chips in `drawWalls` and the amber
  floor-edge `W × D` label were CONSOLIDATED into this one site.
...
Category tabs (16 since the 2026-08-09 Plants tab — NB the earlier "16" claim was off by one, measured 15; the 2026-08-07 split's Furniture tab became
Seating/Tables/Bedroom/Storage/Decor and the stairs-cat cards joined Structure; the
"Vehicles" tab (id `'vehicle'`, 2026-08-06 merge, user-requested) carries the
`vehicle`-cat furniture kinds (car, ev_charger) FIRST
plus every loaded+active vehicle-pack model after; with no pack loaded it shows just the
two furniture kinds — the old conditional pack tab (retired id `'vehicles'`) is GONE;
furniture cats via `furnitureCat` + Lights + Controls &
...
tool changes via the config channel. Test `toolbar-test.html` (97/97 — incl. the tab-count pin I10 (16), the Plants-tab card matrix, and the Theater-tab projector-card pins).

### Collapsible sidebar sections
Every collapsible section renders through `Sidebar._section(slug, title, bodyThunk, opts?)` (light-DOM wrapper: clickable `<h3 class="collapsible-header">` + `▸`/rotated arrow; the body thunk is only invoked while expanded) — **EXCEPT the floor picker (2026-07-30 sidebar reorg, user-directed)**: a deliberate NON-collapsible plain `.section` at the very top — COMPACT display-ordered click-to-switch rows (name + W×D + dim `(peek)`/`(disabled)` suffix, ZERO buttons) + "+ Add floor". **Layout order**: Floors (pinned) → **`floortools`** (NEW section — ALL per-floor controls: ✎ size/name modal, Order ▲/▼ (`moveFloor` on the CURRENT floor), Visibility cycle (👁/peek/🙈), elevation, HA-floor bind, Rotate/Move plan, imperial, lock, "This floor's 3D look" incl. yard fill) → a **Collapse all / Expand all** row (writes/clears the module-level `SECTION_SLUGS` — 41 top-level slugs ONLY, room-group `<slug>/<roomId>` sub-keys untouched, the pinned floor picker exempt) → Layers → **Rooms (moved up 2026-08-01, user-directed — per-room flooring made it a top-of-list concern)** → Dimensions → Rulers → **Tools (GROUPED via `TOOL_GROUPS`/`_toolGroups()`** — Select & edit / Structure (wall+kind picker+wall-editing prefs+lock-all) / Areas & ground / Devices & sensors / Furniture & decor; uncategorized ids land in a trailing "Other" safety-net group, so ADD NEW TOOLS TO `TOOL_GROUPS` too) → the rest in prior relative order. Collapsed keys persist **device-local** in `localStorage['diorama:sidebar:collapsed']` (JSON array, try/catch-guarded — NOT the HA store); absent from the set = expanded (default). Section keys are stable slugs — the former `floors` slug is stale-harmless; `floortools` joined the set. The former `scene3d` / `weather` / `data` sidebar sections MOVED into the tabbed settings drawer (Display / Weather / Data tabs — see "Settings drawer & avatar packs"); the per-floor `look3d` overrides live in Floor tools. Test `sidebar-org-test.html` (`SIDEBARORG PASS 175/175` — §M pins the three-way furniture-list split; NB its I2 hotkey pin needs a `sleep(1120)` past HOTKEY_BLUR_GRACE_MS after any focus-blurring section, the sensor-focus C5 remedy). **2026-07-30 polish wave**: the
Collapse/Expand-all row sits directly BELOW the floor picker (above Floor tools). **Floor
LIFECYCLE moved to Settings ▸ "Floor Plan"** (the `data` tab RENAMED — label only, slug
`'data'` unchanged, no caller passes `{tab:'data'}`): per-floor rows (inline name + W/D
inputs, 🗑 with a named `confirm()`, disabled + guarded on the last floor) + "+ Add floor",
ABOVE Configurations; the sidebar picker carries a pointer line and has NO add/edit/delete/
lock/imperial controls. **`Floor.boundsLocked` gates NOTHING anymore** (UI removed; the
canvas edge-drag ignores it — else already-locked floors would be stuck with no unlock;
field round-trips harmlessly, commented at all three sites). **`<diorama-floor-modal>` is
ORPHANED** (its two dispatchers were removed; component + app.ts wiring left inert — noted
follow-up). Imperial checkbox → Settings ▸ Display (above Ground level). **Glass headers**:
`.collapsible-header` is a glass pill — EMBOSSED (raised gradient + drop shadow) when
collapsed, DEBOSSED (inset shadows, brighter text) when `.open` — the relief REPLACED the
▸ arrow (`role="button"` + `aria-expanded` + Enter/Space; `backdrop-filter` is declared but
a no-op over the opaque sidebar — the visible glass is layered translucent gradients,
documented in the CSS so nobody "fixes" the blur). `.section-caption` = the flat no-relief
variant for the pinned Floors heading + tool-group captions; room-group headers KEEP their
▸ (that's the section-vs-group visual distinction). Peek visibility button: the monkey SVG
was `display:block` → 2-line wrap; button is now inline-flex nowrap, all three states one
line. Tool hints name the ARMED variant for furniture/light/door/window/ground (+switch
specifics): "Click to place: Oak tree (3000 × 3000 mm footprint)" + concrete bottom-toolbar
picker copy; hint block tagged `data-tool-hint`. **2026-07-31 polish**: `_section()` stamps a
`collapsed` class; the collapsed-pill rhythm is 25 px → 6 px (clears the emboss shadow's
~4.5 px reach). **2026-08-01 (user-reported "expanding restores the space and the header
jumps")**: the 3 px vertical padding now lives on the BASE `.section` rule (both states, no
divider in either) so a toggled header stays put under the pointer; expanded adds only
`.section:not(.collapsed) { padding-bottom: 8px }` BELOW the body — pinned by
sidebar-org L10/L11 (toggle Δ ≤ 1 px, both directions); the pinned floor picker shares the
tightened padding; the mmWave section title is "mmWave Sensors" (the "on this floor"
suffix was the ONLY wrapping title at 250 px — measured); the BOTTOM TOOLBAR's collapse
handle is LEFT-ANCHORED and 40×40 (first child of `.tb-tabs`, OUTSIDE the new `.tb-tabstrip`
scroller so it can never scroll away; 19 px chevron; collapsed dock flex-starts; same button
both directions, pref key unchanged, aria-expanded). SIDEBARORG 175/175, toolbar 93/93. Stale slugs in a persisted collapsed set are harmless. Room-grouped lists (`_groupedList(sectionSlug, …)`) get per-room-group collapse rows keyed `<sectionSlug>/<roomId>` (the "— No room —" bucket = `/none`). **mmWave detail editors are inline**: the selected sensor's per-sensor config editor (`_activeSensorSection()`) and its HA-data block (`_haSections()` — zones / objects / targets / sensor config) render as plain bordered **sub-blocks** directly beneath the selected sensor's row *inside* the `sensors` section (matching how the Motion section edits inline via `_motionItem`), NOT as separate `_section`s. There are no longer `active-sensor`/`ha-sensor` section slugs (stale keys in a persisted collapsed set are harmless).
...
`Store.layers2d` (`Layers2D`) gates `drawAll` per layer and the 3D scene: group-scoped layers (lights/**switches** (own `_switchGroup` since the split — hidden switches also drop out of raycast, like hidden lights)/sensors/motion/env/**info** (info-card plaques, own `_infoGroup`)/zones/targets/geo/weatherFx) flip `group.visible` each tick via `renderer.setLayerVisibility` (hidden lights also stop being raycast click targets; `weatherFx` off also hides the effect group + stops its per-frame particle motion); `furniture` (NON-appliance pieces) + **`appliances`** (appliance-category pieces incl. appliance custom objects — hiding them also drops their anchors/door pivots/nav footprints, same semantics as furniture) + `bg` + `labels` (room-name labels; hiding them keeps `_roomZones` alive for activity/TV room resolution) are gated at build time inside `updateFloor` (they live in `_floorGroup`) and are part of `_keyFloor`. Hiding furniture also removes sit spots, terrain, and stairwell floor holes. The `targets` layer's display label is **"Avatars"** (key unchanged for preset compat). 2D: gates `drawAll` per layer (bg, furniture, appliances, labels, lights, switches, sensors, motion, env, info, zones, targets — absent = on) plus `activity` (default OFF): glow pools where lights are ON / motion is firing, so the built-in "Simple floorplan" preset (everything off + targets + activity) still shows living rooms. Walls always draw; doors+windows ride the `openings` key (2026-07-29). `nameLabels` (default on) is a special case — it gates confident-rig name sprites (3D) + name text under fused/identified dots (2D); the 3D side is NOT a group flip (labels live inside `_targetGroup`) but a per-frame `sprite.visible` gate that `setLayerVisibility` folds into `_showNameLabels` (see Identity fusion). User presets persist in `Store.layerPresets2d`; sidebar "2D Layers" section. **The layer catalog lives in `src/layer-defs.ts`** (pure): `LAYER_DEFS` (key+label+**`cat`** — the sidebar grid AND the card editor's Custom… grid both render `layerDefsByCat()` GROUPED under `LAYER_CATS` section captions: **Labels / Structure & furniture / Ground & areas / Devices / People & presence / Outside world**; a NEW layer needs exactly one entry there incl. its cat), `SIMPLE_LAYERS` (the one shared "Simple floorplan" literal — sidebar + app.ts `?layers=simple` + card-shared all consume it), `DEFAULT_OFF_LAYERS`/`layerIsOn()` (the default-off ladder — activity/vacuumMap/heatmap are opt-IN; both surfaces delegate so they can't diverge). **2026-07-29 layer additions (all opt-out, absent = ON — old presets byte-identical)**: `objectLabels` ("Object labels") — 2D fixture/door/window/furniture NAME text, self-gating via `objectLabelsOn(p)` + the pure EXPORTED `fixtureCaption(showNames, name, badge?, showBadge = true)` (name drops, STATE badge survives — `Porch · armed away` → `armed away`); VALUE readouts (env readings, temps, watts, counts, now-playing) deliberately stay with their fixture layers; there are no 3D fixture name sprites so the key is legitimately 2D-only. **`openingStatus` ("Door & window status", cat labels, 2026-07-30, absent = ON)** — the ONE exception to "badges stay under the fixture layer": door/window OPEN/closed/% badges get their own switch (they have no value readout of their own); `showBadge` is the 4th `fixtureCaption` param, wired at the three pill sites (`pill()` closure + swing-door + window — all three now skip EMPTY captions, fixing a degenerate empty black plate on unbound openings with names off); both keys off = no pill; geometry cues (arcs, closed-position hints, padlocks, slats) untouched; 2D-only by design (3D expresses openness as geometry). `openings` ("Doors & windows") — doors+windows are NO LONGER always-drawn; ONE key for both (3D shares `_doorGroup` with curtains/deadbolts), and `hitDoor`/`hitDoorEnd`/`hitDoorLock`/`hitWindow`/`hitWindowEnd` early-return when hidden (hidden = untappable, the zonesInteractive rule; 3D via `_doorGroup.visible` off the raycast roots). `peekFloors` ("Peek floors") — the 2D onion-skin underlay. `labels` relabelled "Room & area labels" and now ALSO gates ground-area + pool NAME labels (re-homed from `ground`); `nameLabels` relabelled "Person name labels". **`robots` ("Robots", cat devices, 2026-07-31, absent = ON)** — vacuum/mower docks + bodies split off `sensors` (`robots: false` added to `SIMPLE_LAYERS` to preserve the simple look; `hitRobot` early-returns when hidden — the openings rule; see the Robot fixtures section). **2D floor paint**: `drawFloor` fills with the resolved floor look (`Room.floorColor/Tex` per loop → `look3d` → `scene3d` → `'#101820'`) + a cached deterministic 256px/800mm pattern tile for wood/tile/concrete (world-anchored, zoom-gated, try/caught) — **loop-scoped since 2026-08-01: with closed wall loops the fill covers ONLY the loops (per-room look); outside renders as ground** (see the Rooms section's per-room-flooring paragraph); no loops → full-rect fill. Test `layers2d-test.html` (`LAYERS2D PASS 84/84`; NB drawAll carries canvas state between calls — snapshot pages must paint twice).
...
`SolarPanel` (`Floor.solarPanels`, repairFloor + defaultFloor backfill `[]`;
tool `solar` ☀️ in TOOLS + TOOL_GROUPS "Devices & sensors"; free placement,
grid-snap drag, standard delete/identify/lock/translate/rotate coverage;
display-only — nothing to toggle, no raycast kind tag): a motorized ground
panel that AIMS AT THE SUN. **Pure module `src/solar.ts`** (ZERO imports, the
geo.ts idiom, shared app graph + renderer chunk): `compassToPlanDeg`/
`sunAzimuthPlanDeg`/`sunElevationDeg` — **three-view's `_weatherFxState` sun
read now DELEGATES to these** (byte-equivalent, golden-pinned), so the W3
`sunPosition` scene light and the panel agree BY CONSTRUCTION;
`sunPlanFromClock(nowMs, θ)` deterministic local-clock fallback (rise 06:00
east, set 18:00 west, 60° peak; sine goes negative at night — parking needs
no special case); `resolveSunPlan` → `{sun, source: 'entity'|'clock'}`;
`solarAim(az, elev, baseRot)` → `{yawDeg, tiltDeg, parked}` with **tilt =
clamp(90 − elev, 10, 75)** (panel-normal elevation = sun elevation;
`SOLAR_PARK_TILT` 10 near-horizontal stow at base yaw when elev ≤ 0).
`rotation` is a BASE yaw the tracking composes onto. **3D** `_solarGroup` +
`updateSolarPanels(panels, stateProvider, sunAz, sunElev, uvIndex)`: footing
+ post + YXZ head (`rotation.y = −yaw·π/180`, `rotation.x = tilt·π/180`,
test hooks `userData.solarHead/yawDeg/tiltDeg/parked`); PV face + cell grid
are flat `MeshBasicMaterial` (documented `_mat()` exemption — toon bands
read as artifacts on dark glass); 4 frame rails tinted/emissive by
`weather.ts uvBand` (the SAME WHO ladder as the chip's UV row,
`userData.solarUvColor` hook); power bead glows `powerGlowScale(|W|)`,
AMBER on a negative reading (grid draw vs generation); grade-following via
`_itemGroundY`. **NO advance loop** — aim is built into the mesh under
`_keySolar` = `configRev | floorId | sensors layer | 3°-bucketed az:elev |
uvBand | per-panel(id:x:y:rot:hidden:100 W bucket)` (sun moves <1°/4 min ⇒
a rebuild every ~12 min). `powerEntity` stays LIVE-path (chatty — the
plug/furniture precedent; key folds the bucket per tick). **2D**
`drawSolarPanels` (sensors layer): aim-rotated rect with depth foreshortened
by cos(tilt), UV-band frame stroke, W chip (amber negative), sun arrow out
the face, dimmed arrow-less when parked. Sidebar `_section('solar', …)`:
...
**Ground-stack depth layering (2026-08-06, user-reported "banding" on the zoo-grounds plan — the coincident-face gotcha at PLAN scale)**: areas painted OVER other areas at the same elevation all landed at exactly y=4 — literally ZERO depth separation, so the rasteriser interleaved them per pixel along triangulation diagonals (dark hatched wedges sweeping the whole yard; worst on plans that layer paint — zoo 19 areas, garden-center 10). NOT a precision shortfall you can nudge away: one depth-buffer LSB at the stock frustum (near 10 / far 150000) at ~24 m camera distance is ≈3 mm — the same order as the entire 2/4/6/8 mm ground stack, so world-space y-nudges small enough to preserve the stack cannot resolve. Fix is **polygonOffset by nesting level**: `groundDepthLevels(areas)` (level = 1 + max level of earlier same-elevation bbox-OVERLAPPING areas, strict bounds so seam-to-seam neighbours share a level; whole library max depth 3) → `groundDepthOffset(level)` = negative polygonOffset (factor AND units — units-only failed AT THE MAGNITUDE TRIED (−1/−2 per level; re-measured 2026-08-06 with a two-pose temporal probe: −8/level resolves ties at normal elevations but sits ~3× the noise floor at grazing el 8 and degrades under the neighborhood-widened frustum — the slope-scaled factor is load-bearing). Deterministic `renderOrder` painter-ordering is NOT a substitute: depthFunc is already LessEqualDepth, and per-pixel interpolation jitter between differently-triangulated coplanar polys makes the later polygon fail the depth TEST (measured 6.4% static error) — draw order cannot fix a test failure. Verified TEMPORALLY as well as statically (2026-08-06, user-reported 'flashing when moving the camera' — the depth fix was NOT the cause, it IMPROVED moving-camera stability 9.6×–79× across an 18-azimuth orbit and holds at the flights/neighborhood-widened frustum where the pre-fix state was ~2.5× worse; the observed flashing localized to the WALL-CUTAWAY cross-fade — masks showed every flipping pixel on wall faces, none on ground, and disabling wallCutaway cut the user-plan pixel churn 73×): terrain-test §M pins a 0.02° two-pose ground-region agreement (0.028% changed with the fix vs 7.2% without, negative-controlled) on each patch + its skirt, clamped `GROUND_DEPTH_LEVELS` 8; level 0 = no offset (single-area floors byte-identical). Two same-stack co-fighters fixed alongside: the MOTION-sensor coverage cone base sat at y=0 exactly coplanar with the slab → both coverage decals now share `COVERAGE_DECAL_Y = 12` (was already the mmWave wedge's literal); LIGHT floor pools sat at y=3, 1 mm UNDER ground paint → `LIGHT_POOL_Y = 7` + `LIGHT_POOL_DEPTH_BIAS` polygonOffset (deliberate behavior change: outdoor lamp pools now wash OVER painted ground/rugs, matching the 2D paint order which always drew lights after ground). Verified clean at the stock AND ceiling-widened frustum (near 900 / far 13.5e6). Known LATENT sibling (deferred, no repro): `yardFill` (y = 2+gl) is exactly coplanar with the neighborhood overlay's water layer (y=2 in the grade-offset group) — same bug class if both are ever run together. Moved pins: firepit B5 + terrain H7/H7b pool-y 3→7 (the invariants those tests guard are unchanged).

**Vertex insert & whole-shape drags (2026-08-01, user-requested "how do I add a point / drag all the points together")**: every polyline/polygon with vertex-drag machinery gets midpoint **INSERT handles** when editable (edit+Select, `insertHandlesEnabled`; dim "+", 0.7× the real-handle hit radius, edges <24 px screen skipped; pure `midpointHandles(pts, closed, minLenMm)` + `VertexInsertHandle` in geometry.ts; painter `drawInsertHandles`; hit fns `hit*Insert` run AFTER their real-vertex test so a real vertex always wins) — mousedown SPLICES the vertex and starts the ordinary vertex drag (release = one undo step; no save at insert). Coverage: walls (every unlocked wall, N−1 open-polyline handles; spliced vert rides `resolveWallPoint` + release weld), ground (active area, 20-cap), path-backed ground (CENTERLINE handles, 40-cap, regen live), pool (20), void (12 — at-cap shows nothing), pzone (uncapped); rulers/LD2450 zones out of scope. **Whole-shape body drags** `groundMove`/`pzoneMove`/`poolMove`/`voidMove` (+ walls' pre-existing move): first body press SELECTS (contract unchanged), a press on the ALREADY-selected body translates EVERY point by one `snap(delta, 10)` (shape bit-rigid; off-grid outlines preserved; wallMove keeps its raw delta — pre-existing); path-backed translates the centerline + regens live; locked refuses; release = one undo step; NOT in `alignCandidates` or `liveDimChips` (move drags get no chip); a press on a selected body sets `p.drag` so the following click is swallowed by `dragJustEnded` (standard draggable contract — the flight-card/Valetudo low-priority click consumers already tolerate it). Test `vertex-insert-test.html` (`VERTEXINSERT PASS 110/110`).
...
- **Outdoor FurnitureKinds** (`outdoor` cat — the optgroup label entry was also missing and is now added, surfacing the bins too; NB 2026-08-09: `tree`/`pine_tree`/`bush`/`flower_bed` + the five 2026-07-28 tree species MOVED to the `plants` cat — see the furniture-cat paragraph): `bird_bath`, `fountain` (static translucent water column — no particles v1), `swingset`, `lawn_chair` (`seat: 380` — real SitSpot, avatars sit outdoors), `picnic_table` (plain `surface` + `activity: eat_at_table`, NO seat — a centered SitSpot would land on the tabletop; sit at it via lawn_chairs). Standard blob shadows + outlines. Test page `yard-test.html` (YARD PASS 4/4).
- **Tree species + adjustable height (2026-07-28)**: five more tree kinds (cat `plants` since 2026-08-09) — `oak_tree` (thick trunk, 4-lobe canopy), `birch_tree` (white trunk + band rings, airy light-green clumps), `palm_tree` (curved 3-segment trunk — BASE segment vertical so it sits flat — 7 fan fronds + spear), `willow_tree` (squashed dome + 8 hanging drapes), `spruce_tree` (5 stacked blue-green cones, tier offset DERIVED so the tip lands on HT at any height). All authored as PROPORTIONS of (W, D, HT); `TREE_KINDS`/`isTreeKind` (geometry.ts — a NEW tree kind needs membership there to get the height override) + `treeHeightMm(fu, defHt)` (honours `Furniture.ht` on tree kinds only, clamp 1000–15000). **`Furniture.ht` is now a SHARED per-piece field with two DISJOINT consumers** — `stairsRiseMm` (stairs family) and `treeHeightMm` (tree kinds); every other kind still reads `def.ht`. Sidebar "Height (mm)" row on tree kinds (stairs "Rise (mm)" pattern, same slot, disjoint gates). Legacy `tree`/`pine_tree` are refactor-pinned byte-identical at defaults (goldens in tree-test §E; `tree` is AFFINE in HT — crown rises 0.42·Δ; `pine_tree` crown = 1.04·HT); canopy radius capped by `span·0.85` (fronds `py·0.9`) so a squat wide tree never sinks lobes under the lawn. Test `tree-test.html` (`TREE PASS 74/74`).
...
- **Fences/hedge**: `WallKind` + `fence_picket` (rails+posts+flat pickets,
  ...
  SUB-INTERVAL — each run gets its own end posts = real gate posts — and the 2D
  picket ticks/chainlink hatch/railing balusters gained an `inSolid(t)` guard) via
...
- Test page `yardlife-test.html` (`YARDLIFE PASS 59/59`).
...
- **Pool / spa** (`Pool`, `Floor.pools`, repairFloor + defaultFloor backfill `[]`): a
  ...
  three-view `_keyPool` = configRev + per-pool heater-state/pump-on/light-bitmask/
...
- Test page `path-pool-test.html` (`PATHPOOL PASS 62/62`).
...
- **Presence zones** (`Floor.presenceZones`, repairFloor backfill; FP2/Frigate-style): user-drawn ≥3-vertex polygons bound to a binary_sensor (NB the old "3–12" claim was never enforced — actual caps: pzone unlimited (`POLY_VERTEX_CAP_PZONE = Infinity` documents it), ground 20, pool 20, void 12, path centerline 40; geometry.ts constants). Draw latch `drawingPresenceZone` (click verts, dblclick/Enter finish, ESC cancel — wall-draw idiom); selected zones get draggable orange VERTEX handles (`pzoneVert` drag) + midpoint INSERT handles + a Redraw button; body click selects, a press on the ALREADY-selected body starts a whole-shape `pzoneMove` drag (see "Vertex insert & whole-shape drags"). 2D outline (dashed unbound) + occupancy fill glow, 3D flat ShapeGeometry patch at y≈8 glowing when on — both ride the `zones` layer (+`zonesInteractive` gating); `_keyPzones` = configRev + bound states; ids config-path.
...
- **Eight door kinds (2026-07-28)**: `Door.kind` = `swing` (default) / `garage` / `gate` +
  ...
  (`DOORKINDS PASS 123/123`; combined canvas-render+canvas-hit bundle — same flightHitPx-style
...
- **2D door swing arc sweeps the TRUE opening side (2026-07-28 fix, user-reported)**: a
  ...
  of the panel (3D swing + the 2D open panel were always right; they share `doorOpenDeltaDeg`).
  The arc also sweeps the SIGNED 90° from the closed panel via `ctx.arc(..., closedA, openA,
  openDelta < 0)` — the old `min→max` endpoints drew the 270° complement whenever the two
  angles straddled the atan2 ±π seam. Pinned by ruler-dims-test §12 (diff-based pixel
  classification by canvas angle: right hinge, left hinge, seam-straddling left@135°).
...
- **Garage doors**: `Door.kind: 'garage'` — 5 horizontal slats in a 2100 mm opening (`WallOpeningCut.head` carries the taller lintel); openFraction lifts the bottom edge and folds slats past the lintel flat onto a ceiling track. 2D: retracting dashed line + % pill. Sidebar Kind dropdown auto-bumps default w 800→2400 on switch. **Six STYLES (2026-08-06, user-requested)**: `Door.garageStyle?: GarageStyle` (`sectional | raised_panel | carriage | roll_up | glass_panel | tilt_up`, item-level — no repairFloor; absent = `sectional` = the pre-feature 5-slat build BIT-IDENTICAL, golden-pinned; rides configRev → `_keyDoors`, no new dirty-key input; garage-only "Style" dropdown in the sidebar door editor — picking Sectional `delete`s the field; toolbar door chips stay 8 kinds). The 3D branch switches behind a shared `placeSection()` reproducing the shipped roll-up parametrization verbatim: raised_panel = 4 proud embossed insets per slat; carriage = wood tone + centre seam/crossbuck/handles/strap hints (decorations ride their slat); roll_up = coiling drum + jamb guide rails, NO ceiling fold — slats past the lintel simply aren't drawn (in the coil); glass_panel = framed sections, panes 20 mm lapped 8 mm under 44 mm bars (doorGlassMat idiom — transparent panes auto-skip outlines); tilt_up = ONE-piece head-pivot slab (`rotation.x = π/2·frac`, landing flat toward local −Z, the same side sectional folds to). **Garage LOCKS are no longer excluded (the old "garage draws no padlock" rule is GONE everywhere)**: 3D calls the shared `addLockBolts` as a bottom-rail T-handle riding the leaf, gated `frac < GARAGE_LOCK_MAX_FRAC` (0.6 — a mostly-open door's lock is meaningless and the bolt would float at the ceiling; tilt_up parents the bolts to the pivot); 2D draws the standard `lockGlyph()` padlock; `hitDoorLock` accepts garage (display-mode skip retained); the lock bolt beats the panel in the 3D raycast like every other kind. **Open-% visualization (all styles)**: a camera-facing `_makeTextSprite` % badge above the opening while `0.02 < frac < 0.98` — `_doorGroup` carries sprites now, so `updateDoorsWindows` gained the `_disposeSpriteMaps` pairing before its `_clearGroup`; repaint cadence = the existing 5 % `openKey` bucket (accepted) — plus a 2D progress bar stacked ABOVE the % pill (below would smear into the 5 px door stroke — caught empirically, commented at the site). opening-click-test's old `garage builds NO deadbolt` assertion is INVERTED (count preserved 39/39). **NINE styles + hardware + height + colors (2026-08-07, user-requested)**: `GarageStyle` += `sectional_windows_top | sectional_windows_left | sectional_windows_right` — the sectional build + glass lites (doorGlassMat idiom; slab 60 / glass 84 / muntins 100 — three distinct depths because a 20 mm pane INSIDE a 60 mm slab would be invisible, panes 8 mm oversized so edges die inside the bars): top = 4 panes across the TOP slat (the row begins 4/5 up, the classic top-lite door); left/right = one pane per slat stacked at one end. **Chirality**: the exterior face is door-local +Z (sections fold to −Z = interior); an outside observer's LEFT is local −X (`sideSign` −1) — documented at the builder + world-space-pinned at rotation 0 with both doors in ONE scene. **Front/back cues**: the sectional family builds interior tracks (side channels + ceiling rails + curve + opener rail + motor head, all at local −Z), roll_up a drum-end motor, tilt_up side pivot arms, and EVERY style an exterior handle at +Z bottom-centre (hidden on roll_up at frac ≥ 0.99 — its curtain coiled away); all hardware tagged `userData.garageHw` and the test goldens filter it via `leafMeshes()` (the equality goldens — absent ≡ sectional, unknown ≡ sectional, absent height ≡ 2100, garbage color ≡ absent — still compare the WHOLE assembly). **Height**: `Door.garageHeight?` via pure `garageDoorHeightMm()` (clamp `GARAGE_HEIGHT_MIN/MAX` 1800–4200) — wired into `wallCutsForSegment`'s per-door `head`, the build H, lift/drum/pivot/badge/track ys; 5 slats scale; sidebar "Opening height (mm)". **Colors**: `Door.color?` tints the panel/slab/leaf across ALL door kinds (carriage's wood tone replaced; open state keeps a REDUCED green emissive 0x1b5e20 @ 0.18 vs stock 0.35 so state still reads); `Window.frameColor?` tints frames/sashes/mullions/bay casework; glass, shades, curtains, lock bolts, hardware, and the pocket mouth jamb NEVER tinted (asserted); hex validated via `validHexColor` (garbage ≡ absent); 2D closed/neutral strokes adopt the color — open green/unavailable red always win (2D color is state language first); sidebar shared `_colorRow` (door "Color", window "Frame color", ✕ deletes). Tests: `garage-test.html` (`GARAGE PASS 264/264`), door-kinds 123/123, window-test 73/73, curtain 36/36, covers 22/22 (its `slatsOf()` filters hardware too).
...
`_nearestFreeCell`'s largest-region preference used to jump WALLS (a raw point in a wall-backed bookcase footprint snapped to the big outdoor region → rig locked outside). `_buildNav` now precomputes `_nav.wallSolids` (solid wall runs, DOOR openings excised via `wallCutsForSegment.navSolids` (windows are LOS-solid since 2026-08-05), invisible walls excluded — consistent with the rasterizer) and `_nearestFreeCell` filters ring candidates to those with clear wall-LOS from the query point BEFORE the largest-region tie-break, falling back to unfiltered only when every candidate is walled off (never fails where the old code succeeded). `_regionOfWorld` (and through it the stuck-respawn + radar/AI goal-region resolution) inherits; `_nearestFreeCellInRegion`/`InLoop` were already constrained-safe. Door openings pass LOS (windows do NOT), so walking out through a door still tracks. Regression: `test-pages/bookcase-los-test.html` (`BOOKCASE-LOS PASS 11/11`).
...
- **Safety kinds `gas` + `leak` (+ `glass_break` 2026-07-30)**: sidebar section retitled "Safety sensors" (slug `safety` unchanged). Gas = ceiling ring beacon (amber-green). Leak = FLOOR puck (y≈15, `safetyIsFloor`) whose alarm grows a blue puddle ~30 s — growth curve is the shared pure `leakPuddleGrow`/`leakPuddleRadiusMm` (geometry.ts; sqrt ease-out from `LEAK_PUDDLE_MIN` 18 % — the old linear-from-zero curve was an invisible speck for seconds, and the gallery GIF captured a fixed-size patch until `capSafety` learned to ramp the age via `updateSafetySensors`' optional trailing `leakAgeS` provider) — (2D spreading ellipse + 💧; 3D flat decal on the shared `_puddleTexture` — never disposed per-fixture); rides the existing force-rebuild-while-alarming key. **`glass_break`** = a SQUARE mic plate (`safetyIsPlate`, blue-violet `#7e6bf5`, grille slots, `GB` glyph); alarming adds 2 spiky 8-point-star shatter bursts alongside the standard rings (no new per-frame system). **`siren`** (SHIPPED Phase 1 2026-07-17 — a blue ceiling beacon w/ sweep + strobe + rings, `Planner.triggerSiren`; was never documented here) gained its CONTROL SURFACE 2026-08-05: `triggerSiren` dispatches BY STATE for a bound `siren.*` (`siren.turn_off` when sounding, else `siren.turn_on` — the valve open/close precedent, never a blind toggle) carrying the fixture's `tone`/`volume`/`duration`, each **feature-gated** via the pure `SIREN_FEATURE`/`sirenSupports`/`sirenTones` (list AND dict forms)/`sirenTurnOnData` (geometry.ts — an unadvertised param is never sent, the tuya-local #2980 hazard); `switch.*` keeps the generic toggle, binary_sensor display-only, unbound flips localState. `SafetySensor.allowControl?/tone?/volume?/duration?` (item-level); `allowControl:false` refuses in EVERY mode through the one `triggerSiren` choke point (the `lockControl:'display'` precedent — covers 2D/kiosk/3D/sidebar at once); sidebar tone/volume/duration controls appear only when the bound siren advertises the flag. Test `siren-test.html` (`SIREN PASS 45/45`).
...
`RobotFixture` (`Floor.robots`, repairFloor + defaultFloor backfill `[]`; tool `robot` 🤖, sidebar `_section('robots', …)`): the placed x/y is the DOCK. Kind `vacuum` binds `vacuum.*` (VacuumActivity), `mower` binds `lawn_mower.*` (LawnMowerActivity) + optional GPS: `trackerEntity` (device_tracker with latitude/longitude attrs + optional `direction` heading — Mammotion `<name>_gps` shape; its gps_accuracy is hard-coded 0, never draw accuracy rings) OR `latEntity`+`lonEntity` sensor pair (tracker wins). All bound ids config-path in `_isSlowEntity`. **Movement controller lives in the PLANNER** (`Planner.stepRobots(dt)`, advanced from the 2D RAF right after `stepLerp`; `Planner.robotStates` is the single source of truth read by BOTH `drawRobots` (2D) and three-view→`updateRobotRigs` (3D) — a robot moves even if 3D was never opened). Steering: the VACUUM is **straight-line LOS with wall avoidance, NOT A*** (goals validated/re-picked against the pure `segCrossesSolidWall` — WHICH READS `wallCutsForSegment.navSolids` since 2026-08-05 (windows are solid to robots; a visual consumer must ask for `solids` explicitly, documented at the fn); serpentine wiggle ~0.30 m/s — byte-identical pins in robot-test). **The MOWER is a BICYCLE MODEL (2026-07-30, user-reported "needs to move more like a car")**: pure `stepBicycle`/`mowerWaypointReached`/`wrapAngle` + `MOWER_KINEMATICS` (geometry.ts — cruise 420 = ROBOT_DEFAULTS.mower.speed, floor 100, `turnRadiusMm` 500, `ω = clamp(k·err, ±v/R)`, accel 700, brake-to-park on `stop` targets); position advances STRICTLY along heading (never strafes), `RobotState.speed` added; EVERY branch (GPS chase, dock return, sweep, ellipse) routes through it; row pitch = `MOWER_ROW_MM` 1200 (≥2R) via `mowerSweepWaypoints`' trailing optional `rowCell` (legacy calls byte-identical); waypoints advance on pass-by; **inside-the-turning-circle goals hold the wheel straight until the target falls outside** (else the arc orbits forever); renderers read the kinematic `rs.heading` for free. GPS mode: the projected fix is the carrot under the same kinematics; mower in **GPS mode** (bound + `geoFit()` quality ≠ none), else **simulated** boustrophedon over cells outside all wall loops (ellipse-ring fallback when loops fill the rect). Unbound = autonomous demo (run 90–180 s → return → dock 60–120 s, hash-desynced; runs in kiosk/view). Click (2D + 3D `userData.kind='robot'`): bound → `vacuum.return_to_base`/`lawn_mower.dock` when active else `vacuum.start`/`lawn_mower.start_mowing`; unbound → flip run↔return. LED/state palette via `robotLedColor` (cleaning/mowing green + spin/bob, returning blue, docked amber breathing, paused amber, idle dim, error red blink). 3D: docks build under `_keyRobots`; rigs are persistent per-frame objects (`_robotRigs`, `_robotRigGroup`) mutated in place like humanoids; both ride their OWN **`robots` layer** (2026-07-31, split off `sensors` — `Layers2D.robots` absent = on, `robots: false` in `SIMPLE_LAYERS` to preserve the simple-floorplan look; `hitRobot` early-returns when hidden = untappable, 3D raycast free via `.visible`; `_keyRobots` needs no flag — setLayerVisibility runs per tick). **Position-info overlay** (`RobotFixture.showPosInfo?`, item-level, absent = off; "Show position info" sidebar checkbox): `Planner.robotPosInfo(r)` (runtime, per-frame-safe; exported `RobotPosInfo {mode:'live-map'|'gps'|'sim', rawText, worldX/Y, headingDeg}`) resolves vacuum raw→`vacuumRawToWorld` / mower GPS fix / sim fallback; 2D `drawRobotPosInfo` draws a crosshair at the PROJECTED (un-eased) reported point + dashed connector to the drawn body when >50 mm apart (`ROBOT_POS_DELTA_MM`) + a 3-line monospace plate (raw / → world mm / mode · Δ); 3D deliberately skipped v1 (2D-first diagnostic, recorded-pins precedent); both kinds get a `→ x, y mm · mode` sidebar readout. **Manual calibration nudges** ("Align position" sidebar row — vacuum with posEntity/valetudoId, mower with a GPS binding): ←↑↓→ adjust `posOffsetX/Y` by the shared `diorama:moveStep` (↑ = +Y world), vacuum-only ↺/↻ 0.5°/5° on `posRotDeg`, Reset clears; one undo step per click. The MOWER now honours `posOffsetX/Y` as a world-mm GPS TRIM — `_mowerGps` adds it to the projected fix BEFORE the inYard/`clampToBoundary` check (trim participates in the yard clamp); see the plan-rotation sign-exception note (mower trim = vector). **"Calibrate to dock"** (mower GPS block button): `Planner.calibrateMowerToDock(r)` — park the mower on its dock, click → solves the trim from the UNTRIMMED/unclamped `latLonToPlan` projection so the reported fix lands at the placed dock (r.x, r.y); refuses (false) on non-mower / no fit / no fix; saves itself = one undo step. **Dock rotation + front** (`RobotFixture.rotation?`, repo screen-CW deg, BOTH kinds; sidebar "Rotation (°)" row; `rotateFloorContent` robots loop `bump(it)`): at rotation 0 the dock OPENING faces world −Y (front = local −Y ⇒ `frontVectorPlan(rot) = (−sin r, −cos r)` — the pre-existing 3D build direction, byte-identical at 0); 3D `grp.rotation.y = −rot·π/180` (the `_buildFurniture` yaw idiom) + an emissive entry strip on the base's front edge (both kinds); 2D rotated dock rect + an ALWAYS-drawn front chevron at the opening; hit tests layered rotated-rect OVER the legacy centre circle (strict superset). Pure `dockParkedHeading(rot)` = **`+frontVectorPlan` — nose OUT of the mouth** (0 → −π/2, 90 → π, 180 → +π/2, 270 → 0): mower SPAWNS parked and the docked branch EASES yaw onto it under the bicycle yaw ceiling (never spins in place); vacuum spawn heading stays 0 (controller goldens pin it). **REVERSED 2026-08-09 (user-reported "the mower is docked 180 degrees incorrectly in the dock so it ends up intersecting the wall")** — the original convention aimed the nose INTO the dock, i.e. at its back wall and whatever the dock backs onto; measured on the real rigs the 3D mower's front reach is 276 mm against a dock back-wall inner face at 230 mm, so the chamfered nose sat 46 mm inside it. **The heading was only HALF the fix**: the drawn rig is ~546 mm nose-to-tail (nose, drive wheels and skirts all overhang the nominal 450 mm `bodyD`) while the dock's usable depth is ~494 mm, so a centre-parked body buries ~40 mm of itself whichever way it faces. Pure `dockParkedPoint(r)` + `MOWER_DOCK_PARK_OFFSET_MM` (60) seat a MOWER that far forward along the front vector (rear reach 270 − 60 = 210 < 230, clear at every rotation, nose poking out of the opening like a real mower in its carport); a VACUUM returns the dock ORIGIN unchanged (round puck, no nose, symmetric overhang — golden controller probes pinned). Applied at THREE sites — `_spawnRobot`'s pose, the docked/returning steering target `home` (clamped FROM the parked point, so ease / arrival / the relocation snap can never disagree) and canvas-render's no-controller-state fallback; the offset is far inside `arriveMm` (420) so every "arrived at its dock?" test is unchanged, and `calibrateMowerToDock` deliberately still targets the dock ORIGIN (it calibrates a fixed user-placed landmark and 60 mm is inside GPS noise). NB rotating a dock now moves its parked point too, so a docked mower drives a short re-seat leg (still never a spin in place). The rigs' forward axis was measured CORRECT in BOTH renderers (drawn nose ≡ `RobotState.heading`, dot 1.0000 at 0/45/90/180/270/117°) and is now permanently pinned — this was a parking-convention fix, NOT a cover-up for a forward-axis error, so never "fix" a parked-pose complaint by flipping the driving convention. **2D mower body is a heading-rotated rounded RECT** (600×450 matching the 3D box — `bodyD` 450 is ALONG travel, `bodyW` 600 across; `ctx.rotate(−heading)`, local +x = forward; nose bar replaces the centre tick; LED/progress-arc/label anchor off the half-diagonal; parked-heading fallback when no controller state); vacuum body stays a circle. **Mower OUTDOOR CONTAINMENT (2026-08-05, user-directed)**: EVERY position-writing mower branch routes through `Planner._mowerAdvance` — a step commits only if LEGAL (endpoint not inside a BUILDING loop + segment crosses no nav-solid run; `wasInside` forgives an already-indoors mower so it can leave through a door but never cross a wall) AND CLEAR (an 800 mm forward probe ≈ turn radius + body length — the PREDICTIVE half that makes deflection kinematic); illegal → re-step toward the smallest-deflection escape bearing from a FIXED fan (0..±145,180° — no Math.random), else brake + wheel toward escape at the min-speed turn rate (the station-keeping discipline; max Δθ 0.0397 rad/step < the 0.042 ceiling). GPS: an INDOOR projected fix clamps the CARROT via `nearestPointOutsideLoops` (in `_stepMower`, deliberately NOT `_mowerGps` — `robotPosInfo` keeps reporting the raw fix, the overlay delta stays honest). **GARAGE EXCEPTION (2026-08-08, user-reported "mower parked past the wall instead of inside the dock" — supersedes the old "indoor dock parks at the nearest outdoor point" rule)**: the loop(s) CONTAINING the dock are mower-legal — a real mower docks in a garage/shed/sunroom — gated on REACHABILITY (`_mowerDockUnreachable` = dock in a loop with NO door on its boundary; door test = `doorSpanCenter` within `MOWER_CONTAINMENT.dockDoorReachMm` 200 of the loop boundary via the shared `_mowerDockDoors`; unreachable → byte-identical shipped fallback, park at nearest outdoor point + the amber warning, now reworded to "room with no doorway"). `_stepMower` filters the dock's loop(s) out of the forbidden list (`dockLoops`) for the docked/returning branch AND the GPS clamp (a fix in the shed is honest, both directions); simulated MOWING keeps the unfiltered loops (mowing stays outdoors; `wasInside` forgiveness covers leaving). Wall-crossing stays fully enforced — entry is only ever through the doorway. **Doorway ROUTING (same batch — the greedy deflection steerer DEADLOCKS when the straight line to the dock crosses the room boundary far from the door; probe: corner door + far-corner dock never converged at ANY width 800–1200, stuck at a stable pocket; width was NEVER the issue — an aligned 800 mm default door threads fine)**: `_mowerDoorWaypoint` — when the mower and its target are on OPPOSITE sides of the dock room, the target becomes a point `dockDoorWaypointMm` 700 past the door span centre on the target's side (normal sign by `pointInAnyLoop` probe, 700→300→bare-centre fallback; door picked nearest the DOCK, never the mower — mower-relative oscillates between doors while rounding the building); the side FLIP is the phase transition (both inside → null → normal leg, no extra state); waypoint legs run WITHOUT `stop` (braking at the threshold stalls astride the span); wired into docked/returning, GPS both directions, and the mowing/ellipse EXIT. Known v1 limitation (documented at `_mowerDockUnreachable`): reachability is boundary-proximity, not door-graph — an interior door to another forbidden room false-passes. **Room-exit routing + GPS relocation (2026-08-09, user-reported "the mower has shown as going inside the house and stopped inside" — root-caused on the real 19-room plan: 13/19 in-room starts deadlocked; a mower ends up indoors when walls are drawn/closed around it (`wasInside` forgiveness) or via the one-drag-stale loop cache, and the dock-only router left it with just the greedy escape fan)**: (Prong A) `_stepMower` computes the room the MOWER is standing in per step and `doorWp` routes across THAT room's boundary FIRST (door picked nearest the MOWER via `_mowerDoorWaypoint`'s new trailing anchor params, default = dock so all shipped call sites are byte-identical; standing in the dock's own room keeps the dock anchor — loop-object identity off the shared cache), falling back to the dock-room routing; applies to every branch through the one shared closure. (Prong B) GPS branch only: movement-based stuck accounting (`RobotState.stuckRefX/Y/T`, runtime-only; `MOWER_CONTAINMENT.stuckMoveMm` 300 / `stuckRelocateS` 8 — MOVEMENT, not distance-to-carrot, so a long legitimate detour that keeps moving can never trip it) — a body that hasn't moved 300 mm net in 8 s SNAPS onto the clamped carrot (the real mower IS at the fix; the stuck body is a display error — the humanoid ground-truth-respawn rule; gliding would redraw the drive-through-walls artifact), gated on the carrot being legal (`nearestPointOutsideLoops`'s two-sided fallback can return an interior shared-wall point — there the mower HOLDS, guarded not fixed); stuck fields cleared on every non-GPS step. SIM mowers never relocate (synthetic bodies never teleport across walls — the humanoid synthetic-rig rule); a doored trap that CIRCLES (net movement > 300 mm) is Prong A's job by design, Prong B catches only genuinely stopped bodies (measured + commented). **Prong B also covers the DOCKED branch for BOUND mowers (2026-08-09, user-reported "the mower docking still is not aligned" — the deck-side approach crossed the long wall far from its end and wedged at the corner; docked never takes the GPS branch, so it had no backstop)**: `dockTruth = act === 'docked' && !!r.entity_id` keeps its own stuck accounting (same constants/fields) and past 8 s of no movement snaps the body onto `home` with `dockParkedHeading` — the entity state IS ground truth (the machine is physically on its dock; `home` needs no legality guard, it is the branch's own clamped steering target); `returning` deliberately excluded (mid-journey, no truth point) and unbound demo mowers never snap; the non-GPS stuck-field wipe is gated on `!dockTruth` so each mode earns its own full window. NB the true-stop class only occurs in slots narrower than the turning circle — wider wedges eventually grind free (measured). **`BOUNDARY_WALL_KINDS`** (4 fences + hedge + railing): solid runs (fences contain the mower; gates pass) but their loops are NOT building interior — `buildingWallLoops()` feeds BOTH the guard and `mowerSweepWaypoints` (a fenced yard sweeps as lawn, was 0 waypoints → ellipse fallback), and `canvas-interact.isFenceLikeKind` DELEGATES to the same set. `_mowerLoopCache` keyed floorId:configRev (one drag-gesture stale, harmless, commented). Test page `robot-test.html` (`ROBOT PASS 298/298` — incl. §g/§g2/§h parked pose (nose vs the dock's own MEASURED entry strip, seated body vs the dock's MEASURED back-wall obstruction, 2D pixel-measured nose bar, at 0/90/180/270 + a non-axis angle, each negative-controlled against a full pre-fix source rebuild; NB the 2D nose-bar pixel filter must stay min-channel-based with a ±40 guard — the white bar sits over the state-coloured body stroke, so an amber docked LED tints it and a tight ±14 spread finds nothing), window/door twin LOS, 4000-step every-step containment, GPS clamp + honesty, gate passability, the garage-exception matrix incl. the adversarial corner-door fixture + doorless fallback, §e room-exit/relocation (doored-trap exit w/ router-nulled negative control, doorless trap relocating at exactly 8 s, interior-carrot hold, detour non-regression, sim-never-teleports, anchor-pick matrix), and §f docked-bound relocation (four harness-only negative controls prove every pin discriminates).
...
**Delete key** deletes the CURRENT selection. `Planner.deleteSelection(): boolean` (edit-only) removes the highest-priority current selection — the same removal the delete TOOL performs, but keyed off the active-selection ids, not a cursor hit. Priority: **selected vertex → furniture → mmWave sensor → fixtures (motion/env/ble/alarm/calendar/thermostat/safety/alert/robot/camera/projector/valve/plug/info/action/solar) → presence zone → ground area → void area** (declarative table; robot delete also drops its `robotStates`). Stale active ids (item no longer on the floor) are cleared and skipped; a **locked** top selection refuses (returns false, no crash, no fall-through). A removal `save()`s (→ one undo snapshot) + `emitConfig()`s. **Vertex selection**: `Planner.selectedVertex` (`{kind:'pzone'|'ground'|'void'|'wall', itemId, index}`) is set on a vertex mousedown in canvas-interact (the `pzoneVert`/`groundVert`/`voidVert`/`wallv` drag branches), cleared on any fresh select-tool mousedown, `setTool`, and store load. Delete on a selected vertex removes THAT vertex: polygons refuse when it would drop below 3 points (return false; vertex kept); a wall vertex follows the delete-tool rule (2-point wall → whole wall removed, longer polyline → vertex dropped). Keyboard hotkeys (Delete/Backspace, tool picks, undo/redo) share the `isEditableTarget()` guard (`src/dom-utils.ts`, pure/dependency-free so tests bundle just it) so nothing fires while typing in an INPUT/TEXTAREA/SELECT/contentEditable. **Hotkey master switch (2026-07-30, user-reported "tool selection still keeps inadvertently happening when typing")**: `Planner.hotkeysEnabled` (device-local `diorama:hotkeys`, absent = ON, `setHotkeysEnabled` persists + emitConfig, never save(); Settings ▸ Display "Input" block). The investigated LEAK is STRUCTURAL, not a guard bug: clicking any non-focusable surface (collapsible header, list row, canvas) drops focus to `<body>`, where plain keys are legitimately live — undistinguishable from intentional shortcuts, hence the switch. With hotkeys OFF: tool letters/digits, Delete/Backspace, arrow-nudge are DEAD; Ctrl/Cmd+Z/Y/0, Escape (latch cancels), Enter (latch finish), Space pan-hold stay live (modifier combos + already-armed-latch keys can't fire from typing). Test page `test-pages/undo-test.html` (`UNDO PASS 47/47`; bundles the real `Planner` + `dom-utils` via esbuild + fake HaApi, same harness as config-test). **Typing protection v3 + REBINDABLE hotkeys (2026-08-07, user: "still experiencing tool behavior changes as a result of typing… I also need a method to set key binds")**: (1) focusing ANY editable target in edit mode REVERTS the tool to Select (canvas-2d `_onFocusIn`; setTool already clears every draw/placement latch, and the pending furniture/vehicle/custom kind is inert once `tool !== 'furniture'` — verified, documented at the site) — no armed tool survives into or out of a typing session; (2) `HOTKEY_BLUR_GRACE_MS` 1000 — plain-key bindings are DEAD for 1 s after focus leaves an editable (`_onFocusOut` stamps `_lastEditableBlurAt`; modifier combos/Escape/Enter/Space resolve above the grace check); (3) **`src/keybinds.ts`** (pure, zero-import): `KEYBIND_ACTIONS` catalog (8 `tool.*` + `tool.motionAlt` 'm' + `deleteSelection`/`deleteSelectionAlt`), device-local `localStorage['diorama:keybinds']` (action→key; explicit `null` = DISABLED binding), `resolveKeyAction`; canvas-2d `_onKey` resolves plain keys through it; HARD-CODED (documented at module top): Ctrl/Cmd combos, Escape, Enter, Space pan (e.code), arrow-nudge — all still under the `hotkeysEnabled` master switch, which kills the whole rebindable class above the resolver. Settings ▸ Display ▸ Input "Keyboard shortcuts": per-action rebind capture (button keydown `preventDefault`+`stopPropagation` — a `<button>` isn't an editableTarget, capture would otherwise fire the old binding), ✕ disable, Reset all; duplicate keys rejected. Tests `keybinds-test.html` (`KEYBINDS PASS 65/65`) + sensor-focus §D/§E (NB pre-existing C5 needed an `await sleep(grace+120)` — the grace itself is §D's subject; no production behavior softened).

**Hot-selection gate + placement autofocus** (the "typing a name deletes the sensor" fix): the Delete/Backspace HOTKEY additionally requires `Planner.selectionHot` — a runtime boolean set by `markSelectionHot()` from every interactive selection path (the 23 deletable `setActive*` setters + all canvas-interact selection/vertex sites), reset cold in `_applyLoadedStore` + `setUiMode` — so a PERSISTED selection (`activeSensorId` survives reloads) can never be deleted by a stray keypress in a fresh session (the delete TOOL + sidebar buttons bypass the gate; `deleteSelection` itself unchanged). Root cause of the report was typing into a never-focused field: keys fell through to body → hotkeys. Primary fix: every named-fixture PLACEMENT (18 canvas-interact creation sites) calls `markNewlyPlaced(kind, id)` (runtime `Planner.newlyPlacedFocus`, also heats the selection); `Sidebar.updated()`'s `_maybeFocusNewlyPlaced()` focuses + selects the editor's Label input (tagged `data-label-for=${id}`; ~3-frame retry; SKIPPED under 900 px where the overlay sidebar closes after placement) — typing after placing lands in the name field. Re-selection of an existing item never steals focus. **Focusin cooling (the "renaming a yard area deletes it" fix)**: canvas-2d's window `focusin` listener (`_onFocusIn`) sets `selectionHot = false` whenever ANY editable target gains focus — the in-input keystroke was already guarded by `isEditableTarget`, but the selection stayed hot through a typing session, so a stray blur (tap-away, Tab, tablet keyboard dismissing) + Backspace hit the body and fired `deleteSelection` on the still-selected item. A typing session now DISARMS the delete hotkey until a canvas / sidebar-row re-selection re-heats (the placement autofocus therefore also cools — protective; delete TOOL + sidebar Delete buttons unaffected). NB `setActiveGroundArea` TOGGLES (second call with the same id deselects) and `deleteSelection` is a PRIORITY list where the always-set `activeSensorId` outranks ground areas — both bit the regression test. Tests: `sensor-focus-test.html` (`SENSORFOCUS PASS 28/28` — bundles the REAL sidebar AND canvas-2d (`import './canvas-2d.js'` in the entry): copy `tsconfig.json` into the temp src dir (else esbuild leaves raw decorators → parse error) + `--alias:lit=...node_modules/lit/index.js` + `--alias:lit/decorators.js=...`; fake HaApi needs `states:{}` and the `onState`/`onConn` method names; run with `--window-size=1200,900`); ruler-dims-test grew to 63/63 (hot-flag transitions; later 107/107 with live drag dims + the door swing-arc fix).
...
**Window kinds** (`Window.kind`: `single` (default, legacy one-pane) / `double_hung` / `casement_pair` / `sliding` / `picture` / **`bay` / `bay_bench` (2026-07-30)**) drive the 3D pane composition in `_buildWindows` (all item-level, no `repairFloor`): single tilts outward when open; double_hung stacks two sashes at offset depths split by a horizontal meeting rail and slides the lower sash up behind the upper on open; casement_pair splits a vertical center mullion with two leaves hinged on their outer edges that swing open symmetrically; sliding places two side-by-side sashes at offset depths and slides the movable one behind its neighbor on open; picture is a fixed pane (open = tint only). **Bay family**: three-pane assembly projecting OUTWARD — projection side from the pure `bayProjectSign(w, loops)` (local +Z is the INTERIOR by the shade/curtain-rod convention; loop-probe resolves honestly, no-loop falls back −Z); `bayPlan(W)` (returns 28 % of W, depth min(600, sideRun) ⇒ splay ≤45°); kind-aware defaults via `windowSillMm`/`windowGlassHMm`/`windowDefaultWidth` (bay 450/1500/1800; others unchanged — `wallCutsForSegment` resolves sill/height through these so cut and glazing can't disagree); centre pane is the operable awning sash, returns fixed; CURTAINS GATED OFF for bay kinds; knee wall + head board + roof + corner posts. **`bay_bench`** adds a cushioned window seat registered as a REAL SitSpot in `updateFloor`'s window pass (`win:<windowId>:0`, seatY = sill, front normal INTO the room — back to the glass, `soft: true` so pets curl; SitSpots are NO LONGER furniture-only) and is EXCLUDED from the `gaze_window` anchor list (the standing anchor fought the seat). 2D: protruding trapezoid notch + cushion bar. Mullions/rails are **opaque frame bars, thicker than and overlapping the glass** so their shared planes hide (no coincident-face hatching against the transparent sashes). Sidebar Windows editor adds the kind dropdown + sill/height (mm) inputs. Window kind/sill/height changes ride `configRev` (already in `_keyDoors`).
...
**Curtains are INTERACTIVE (2026-08-08, user-requested)**: `Planner.toggleCurtain(w)`
— bound cover → STATE-PICKED `cover.open_cover`/`close_cover` (the valve
precedent, never a blind toggle), switch → toggle, binary_sensor display-only,
unbound → flip `curtainPos` 0↔100 (edit = one undo step; kiosk session-only;
view refuses); `Planner.curtainFraction(w)` is the one openness rule (mirrors
the renderer's `_resolveCurtainFrac`). 3D: `'curtain'` is IN the click union —
fabric panels stamp `fixtureId`/`entity_id` and resolve BEFORE the parent
window (deadbolt-over-door layering); the ROD stays pass-through→window;
dblclick no-op; Alt+click identifies the window. 2D: the tick is a real hit
target — the exported `curtainTickPx(view, w)` (canvas-render) is the ONE
band geometry both `drawWindows` and `hitWindowCurtain` consume (they can't
disagree); hit priority AFTER `hitWindowEnd` (the band runs the full span
inside the ~10 px endpoint tolerance — curtain-first would make curtained
windows un-rotatable) and BEFORE `hitWindow`; edit mode uses a press-only
`windowCurtain` drag kind (windows toggle at mouseup click-vs-drag, so a
plain click-path hook would be swallowed by `dragJustEnded`); kiosk clicks
+ pointer cursor wired. NB the 2D tick sits on the OPPOSITE flank from the
3D drape (screen perp `(−dy,dx)` = window-local −Z vs the drapes' interior
+Z) — same flank as the coverEntity blind tick; a plan tick is a symbol,
documented at `curtainTickPx`, don't "fix" one in isolation. Bay kinds now
gate the 2D tick too (3D already did — symbol and model agree).
Sidebar Windows editor "Curtain" sub-block. Test `curtain-test.html` (71/71).
...
### 2D pan / zoom
`Planner.viewCenter` (mm in world frame) and `Planner.zoom` are runtime state — **but
device-locally RESTORED on load since 2026-07-31 (view persistence + the boot-framing fix)**:
`src/view-persist.ts` (pure, zero-import — `VIEW_RETAIN_MARGIN_FRAC` + `viewCenterFitsFloor`
MOVED here from planner.ts, re-exported for compat; + `camTargetFitsFloor`, parse/validate, the
pure `resolveBootPose()` boot ladder). Keys `diorama:view2d` `{configId, floorId, cx, cy,
zoom}` + `diorama:cam3d` `{configId, floorId, pos[3], target[3]}` — single-slot, written
throttled 1.2 s w/ rounded-signature compare (2D from the canvas RAF; 3D at `_tickOnce` tail +
a disconnect flush; SKIPPED while autoFollow/cinematicOrbit own the camera), gated on
**`Planner.viewPersist`** (runtime opt-in — app.ts sets it in all three launch paths incl.
kiosk; Lovelace cards leave it FALSE — they share one Planner and are framed by their own
cam/view3d). Restore: 2D in `_applyLoadedStore`, 3D in three-view `_applyBootPose` once
`configIndex` loads, probed once per config|floor, bounds-validated. `resetView()` clears the
2D entry ONLY (it doesn't touch the 3D camera and the writer would re-save within 1.2 s).
**Boot-framing BUG FIX (universal, the "zoomed in a lot on reload" report)**: the renderer
constructor parks the camera at a hard-coded `(0, 9000, −6000)` (~10.8 m) and NOTHING on the
app path ever framed it — `_applyUrlTemplate` returns immediately with no cam/view3d and its
iso call was only the 15 s named-view fallback; bigger plans read progressively more zoomed-in
(20×16 m wants ~35 m). Boot ladder (pure `resolveBootPose`): URL template > user-posed latch
(`cameraGestures()` — a NEW renderer counter fed by OrbitControls 'start' + the pivot-rotate
gesture; view-bar/Sims/saved-view clicks mark posed too) > valid saved pose > iso (or sims
under a card's simsCamOverride). A 20 s reframe latch re-applies default framing when
`floorId|w|d` changes (cached store → HA authoritative body) IFF un-posed — runs at
`_tickOnce` END so `_fw/_fd` are current; supersedes (never fights) `floorSwitchCameraDelta`.
Tests `view-persist-test.html` (**81/82 as of 2026-08-19 — `6d the live pose is persisted device-locally` FAILS at pristine HEAD**, confirmed 3/3 on a `git archive HEAD` harness and unaffected by virtual-time budget 30k/60k/150k or `--run-all-compositor-stages-before-draw`; 6e–6h read/write localStorage fine, so the store works and it is the throttled tick-tail WRITE that never fires. NOT caused by the cursor-zoom work. Root cause UNRESOLVED — either the 3D pose-write path regressed unnoticed or it is environment-specific; the old `82/82` claim here was stale. Open follow-up.), camera-test 88/88. `computeView(canvas, fw, fd, viewCenter, zoom)` derives the actual `View` (`ox`, `oy`, `scale`). Wheel zooms anchored at cursor. Middle/right-mouse OR Space+left pans. Two-finger touch pinches and pans. `Ctrl/Cmd+0` or the bottom-left "⟳ Reset view" button resets.
...
### Smart alignment guides (2D drag) — UNIVERSAL cross-category pool (2026-08-01 rewrite, user-requested)
While dragging in edit mode, the dragged point snaps to align on X / Y independently against a GLOBAL candidate pool — every wall VERTEX (corner), every room ANCHOR, and the CENTER of every placed fixture/furniture on the floor (all ~21 fixture kinds + furniture; polygon/centerline drags (`ALIGN_POLY_DRAG_KINDS`) additionally see other areas'/pools'/voids'/pzones' vertices + path centerlines — fixture drags deliberately don't, so they never feel magnetic to terrace vertices). Doors/windows are neither candidates nor aligned drags (they wall-snap). Enrolled drags (`ALIGN_DRAG_KINDS`, geometry.ts — ONE shared set read by BOTH the snap logic and `drawAlignGuides`; they diverged once and safety/alert/robot/camera/projector snapped with no visible line): the fixture family + `furnMove` + `wallv` + `wallMove` + `roomAnchor` + polygon vertex drags + the whole-shape body moves. Snapping points: fixtures = item center; `wallv` = the vertex measured against the RAW cursor (align OVERRIDES the fired axis of the 15° angle lock — measuring the angle-snapped result could never fire; the other axis keeps the lock); `wallMove` = ALL the wall's own points, min-magnitude shift (a wall IS its corners); body moves = bbox center (one predictable point). Tolerance 8 screen px via `view.scale` (pure `bestAlignShift`, geometry.ts); pool snapshotted LAZILY on the drag's first mousemove into `Planner.alignPool {xs, ys}` (two flat arrays, self ids excluded — ~579 candidates on a 400-piece floor, <1 ms, zero per-frame alloc), dropped at mouseup/`_clearTransientSelection`. Precedence: per-kind move → align (beats grid intent) → release snaps (fireplace/switch wall-lock, wall WELD) still win. **Alt suspends alignment globally** (one coherent free-placement modifier, alongside the wall angle/grid/weld snaps). Excluded: doors/windows, LD2450 zone editor, ruler ends, bg drags, `furnCorner`, and the click-vs-drag control fixtures (alarm/calendar/thermostat/action — their 30 mm modal threshold must not be nudged). `drawAlignGuides` (end of drawAll) draws the dashed lines. Test `align-guides-test.html` (`ALIGNGUIDES PASS 157/157`).

### Furniture canvas rotate handle (2026-08-07, user-requested "double arrow rotate icon outside the corner")
The SELECTED furniture piece (edit + Select, not locked) draws a circular chip (~9·dpr) with a double-curved-arrow glyph floating ~14 px beyond its ROTATED top-right corner (`FURN_ROTATE_R_PX`/`FURN_ROTATE_OFFSET_PX`, canvas-render — published as a pure SCREEN anchor `furnRotateHandlePx`, NOT the envChipHalfPx world-anchor shape: the chip floats a fixed px distance beyond a rotated corner so there is no stable world point; `hitFurnitureRotateHandle` converts the cursor via mmToPx instead; the map clears OUTSIDE the furniture layer gate = hidden layer un-rotatable). Drag kind `furnRotate` (hit BEFORE corner-resize + body): rotation = pointer bearing about the piece centre minus the grab offset (no jump on grab), snapped 15° (Alt = free, quantized 0.1° so the store never records 12-decimal noise); release = one undo step; NOT in ALIGN_DRAG_KINDS (rotation isn't translation). NB an Alt-held UN-MOVED release is the Alt-identify gesture (`altIdentifyAt` abandons the drag) — a real Alt rotation always exceeds the 5 px slop; §18d releases without Alt. Noted follow-up: fixtures with `rotation` but no canvas affordance (projectors/valves/plugs/solar/sprinklers/robots/info/action/calendar/alarm/thermostat plates); cameras + motion + mmWave already have orange handles; doors/windows keep endpoint-drag.

### Alt+click identify (2026-08-01, user-requested "locked items are hard to find")
Edit mode, Select tool, BOTH views: **Alt+click** identifies instead of acting. 2D: `identifyAt()` sweeps the body-level hit tests in select-mode priority order (drag HANDLES refuse locked items; bodies never did — locked items are identifiable; layer-hidden stays unhittable; incl. walls, doors/windows, furniture, every fixture kind, rulers, areas/pools/voids/pzones, room labels, geo landmarks via the new `hitGeoLandmark`). Alt+press falls through to a normal drag (Alt = free placement); identify resolves only if the pointer never moved (5 px slop) at mouseup→click. On hit: the matching `setActive*`-equivalent SET (never toggle) + `markSelectionHot` + the runtime `Planner.identifyFx {kind, id, label, locked, x, y, at}` latch (TTL/fade constants in geometry.ts; cleared on floor switch/`_clearTransientSelection`; NEVER save()d — no undo step) + emitConfig. 2D callout (`drawIdentifyCallout`, last in drawAll, ~3 s fade): kind glyph + label, `🔒 locked — unlock in the sidebar` line when locked. Sidebar: `_maybeNavigateIdentify()` in `updated()` expands the owning slug AND the `<slug>/<roomId>` sub-group (bucketed by the item's own position), scrolls the `data-item-row=${id}` row into view (all 28 list-row wrappers tagged), pulses `.identify-flash` ~1.5 s, once per `identifyFx.at`, never steals focus. Walls navigate to `tools` (bulk lock lives there; no per-wall editor). 3D: three-view records `altKey` via a capture-phase pointerdown (the renderer callback carries no event), maps raycast kinds (`media`/`appliance`→furniture, `lock`→door; `flight` absent) → `identifyItem`, suppresses the toggle/dblclick/vacseg paths; no 3D callout v1 (sidebar nav is the payoff). Edit-only; touch has no Alt (out of scope v1). Test `identify-test.html` (`IDENTIFY PASS 57/57`).

### 3D fixture click

**Glass is PICK-THROUGH (2026-08-10, user-reported).** From ~9 of 384 camera poses around an outdoor fire pit, a window pane at 0.16 opacity won the raycast and silently toggled that WINDOW instead of the fire — and under `wallCutaway` the wall AROUND the pane fades to 0.06 while the pane stays fully clickable, so there is no cue anything is in the way. Ruling: **you can see through it, so you click through it.** Window + door GLAZING carries `userData.pickThrough`, set in exactly three places, all keyed on MATERIAL IDENTITY so the flag can't drift onto a frame: `_buildWindows`' `glass()` helper (single / double_hung ×2 / casement_pair ×2 / sliding ×2 / picture / sliding_glass), its bay `face()` helper guarded `m === mat` (the three bay panes only), and `_buildDoors`' `addBox()` guarded `m === doorGlassMat` (french leaves, sliding_glass panes, the glass_panel garage lites). `_raycastFixture` remembers a hit that reaches its tag THROUGH a flagged mesh as `glassFallback` (`??=`, so nearest-first) and keeps scanning the ray, returning it only when nothing else resolves — so a fixture behind a pane wins AND a pane with nothing behind it still toggles its own window (windows never become unclickable). Frames, sashes, mullions, muntins, meeting rails, bay knee walls/head boards/posts/roof/bench, roller shades, curtain FABRIC (its own `kind:'curtain'` — cloth is opaque) and the curtain rod, garage slats/tracks/hardware and lock deadbolts all stay ordinary pick targets; lock still beats door, curtain still beats window, flights stay last. **Deliberately a FLAG, never an opacity threshold** — two independent reasons, both measured: a light's always-on 400 mm hit sphere is `opacity: 0`, and glass opacity is STATE-DEPENDENT (a CURTAINED pane jumps to 0.42 as the blocked-daylight cue, so a threshold stops treating it as glass and the pane swallows the curtain click — a negative-control run with a `< 0.3` threshold fails exactly that assertion). `_raycastFlightNear` needs no change (screen-space projection pick — it never casts a ray, so occlusion never applied) and neither does `_raycastVacSeg` (casts only into `_vacMapGroup` floor patches, and only after `_raycastFixture` misses). Test `glass-pick-test.html` (`GLASSPICK PASS 21/21`; dist-only harness, no extra bundles).

**STALE PICK MATRICES — the "can turn the firepit on but not off" bug (2026-08-09, user-reported three times; the two prior root causes were WRONG and are recorded below so nobody re-runs them).** A raycast reads `object.matrixWorld`, and ONLY `renderer.render()` recomputes it. A freshly CONSTRUCTED `Object3D` has an identity `matrix`/`matrixWorld` **and `matrixWorldNeedsUpdate === false`** — that flag is raised by `updateMatrix()`, which has never run — so between a rebuild and the next render a rebuilt fixture's world matrix claims it sits at the SCENE ORIGIN and a ray aimed where the user can see it hits nothing. Normally invisible; but three-view **force-rebuilds the light group EVERY frame while a fire is lit** (flicker needs `Math.random()` per frame) and that rebuild runs in three-view's rAF tick — a SEPARATE rAF callback from the renderer's own render — so for most of every frame the newest geometry has never been rendered and a click lands in that window essentially always. **That IS the on/off asymmetry**: an UNLIT pit is not force-rebuilt, so lighting it always worked; putting it out never did. Measured on the reporting user's real store, static camera, one fixed pixel: 60/60 raycast MISS while lit vs 60/60 hit while unlit; end-to-end real clicks 12/12 ON, **0/12 OFF** → 12/12 after the fix; an ordinary `wall_sconce` (never force-rebuilt) 40/40 throughout. The blast radius was every force-rebuilt fixture — a lit fireplace/heatlamp/firepit, an **ALARMING safety sensor**, an **ACTIVE alert beacon**, a flashing logic light: you could not acknowledge an alarm from 3D at the moment it mattered. Fix: `_syncPickMatrices()` (`scene.updateMatrixWorld(false)` + `camera.updateMatrixWorld(false)` — the camera is NOT a scene child so it needs its own call) at the top of `_raycastFixture` AND `_raycastVacSeg`; `_raycastFlightNear` needs none (`getWorldPosition()` self-updates its ancestor chain). Deliberately UNCONDITIONAL rather than flag-guarded: 0.997 ms/call on a 2440-object scene (0.7 % of one frame there), and a "have we rendered since the last rebuild?" flag would have to be correct at every build site. Pinned by `firepit-test.html` §F (`FIREPIT PASS 92/92`) — it rebuilds the light group and picks in ONE SYNCHRONOUS BLOCK so the renderer's rAF cannot heal the matrices (deterministic, not a timing race), F2 asserts the hazard is genuinely present, and both fixtures are deliberately OFF-CENTRE because `_w` maps the floor centre to the scene origin — exactly where a stale matrix puts things, so a centred fixture cannot discriminate. **Two earlier theories, both DISPROVEN — do not revive them**: (1) untagged ON-only flame meshes (raycast-probed: the body Group's backstop tag resolved flame clicks all along); (2) a roaming avatar re-lighting the pit (the reporting user's store has all 8 roamers `enabled: false` and zero motion-avatar/demo rigs — the user-toggle immunity shipped for it is defensible on its own but was never this bug). Related, found in the same hunt and fixed: the tap gate's `lastTapT` seeds at **-Infinity, never 0** (`e.timeStamp` runs from the document time origin, so a 0 seed made every tap in the page's first 350 ms a dblclick — for an unbound light, the entity picker instead of a toggle; same seed and reasoning as canvas-2d's `_lastSyntheticClick`), and three-view resolves every hit through `byFixtureId(arr, id)` (`String(x.id) === id`) because `_raycastFixture` emits `fixtureId: String(...)` — a store with a NON-STRING id was otherwise permanently and silently dead in 3D while still working in 2D, since the 2D path resolves fixtures by ARRAY INDEX (`hitFixture` returns `{kind, idx}`). That id-vs-index asymmetry is the general shape behind "works in the plan, does nothing in 3D"; DUPLICATE ids are deliberately NOT handled at the click site (they key every id-keyed subsystem, so they must be fixed in the data). Known remaining pick quirks, both symmetric in on/off and therefore NOT the report: a ~16 %-opacity window pane can win the pick from ~9 of 384 camera poses (toggling that window instead — the 2D-layered vs 3D-spatial asymmetry is deliberate, but glass gives no cue, and under `wallCutaway` the wall around it fades to 0.06 while the pane stays fully clickable), and a lit firepit costs ~44 % of the frame rate on a large plan purely from the per-frame rebuild (the flicker only needs material mutation — the `_advanceX` idiom would remove the cost AND this whole bug class at the source; noted, not built).
`<diorama-three-view>` registers `click` and `dblclick` on the WebGL canvas. The renderer raycasts into the visible interactive groups **recursively**; the hit-walker climbs parents to the first ancestor whose `userData.kind` is in the fixture-click union (`FixtureClickKind`, now **19 kinds** — tested via `FIXTURE_CLICK_KINDS` + the frozen `FIXTURE_CLICK_KIND_LIST` export so union and walker can't drift). Click → `planner.toggleItem` (bound → `toggleEntity`; unbound → flip `localState`). Dblclick on a `light.*` entity → light config modal; dblclick on unbound → entity picker. Both light **body** and **floor disc** carry the same `userData`. **2D↔3D interaction parity (2026-08-01 full audit, user-reported "doors respond in 2D but not 3D")**: door PANELS (`kind:'door'` on the per-door hinge Group, tagged BEFORE the kind branches so all 8 kinds + outline shells resolve) and WINDOWS (`kind:'window'` on the window group — sashes, mullions, bay casework, shades, bay casework and the curtain ROD all resolve to the one window, mirroring 2D's span-based `hitWindow` — but curtain FABRIC panels are their own click target since 2026-08-08 (user: "curtains should be interactive"): `'curtain'` joined the union, panels stamp `fixtureId`/`entity_id`, and the walker resolves them before the parent window — the deadbolt-over-door layering, no ordering hack) are now 3D-clickable → the IDENTICAL `toggleItem` call the 2D click/kiosk branches make; door/window 3D dblclick early-returns (2D has no opening dblclick — binding is sidebar-only). **Lock beats panel with NO ordering hack**: deadbolts carry `kind:'lock'` on the MESH itself, so the walker resolves them before the parent door — 2D's `hitDoorLock > hitDoor` priority by construction. **No invisible closed-span click-catchers** — 3D clicks the ACTUAL geometry ("click what you see"; a catcher would eat clicks through an open doorway); note the pick-order asymmetry: 2D is layered (fixtures before openings), 3D is spatial (nearest geometry — a lamp seen THROUGH a window pane resolves to the window; orbiting disambiguates). The audit's full matrix lives in docs/STATUS.md's parity bullet; deliberately 2D-only (no device action to mirror): pool heater/pump (sidebar toggles), cameras/calendars/info cards/flagpoles/mmWave/motion/BLE (select/drag only), env dblclick-bind (optional follow-up), all canvas-editing concerns (vertices/zones/rooms/rulers). New kinds ride Alt+click identify (`IDENTIFY_KIND_FOR_CLICK` += door/window) and the touch tap gate for free. Test `opening-click-test.html` (`OPENCLICK PASS 41/41`).
...
- **Light icon kinds** (`LightIconKind` in `types.ts`):
  ...
  - `firepit_round` (◉) / `firepit_square` (▣) — outdoor fire features on the fireplace pattern (2026-08-01, user-requested): shared 900 mm footprint from the pure `FIREPIT_SIZE_MM`/`FIREPIT_KINDS`/`isFirepitKind()` (geometry.ts — 3D/2D/hit all key off it). Round = 9 index-hashed stones (deterministic wobble, NO `Math.random` in geometry — the builder reruns per frame while lit) + ash basin + 3 crossed logs; square = 4 rim runs (side runs overrun so end caps are buried — coincident-face) + boxed basin + logs. ON = ember-bed decal + 4 swaying emissive flame cones + hot core + warm PointLight (raised to the flame bowl) + the standard flicker-scaled floor pool; OFF = dark basin, no light, no pool. Forced warm tint (fireplace idiom); in three-view's per-frame force-rebuild predicate (`fireplace || heatlamp || isFirepitKind`); in `GROUND_STANDING_LIGHT_KINDS` (grade-following, terrain §H7b); builds at ground (`bodyY = 0`, ignores `lightHeight` — the inground/ground_spot precedent, generic Height row stays visible); NO wall snap (snapFireplaceToWall is `iconKind === 'fireplace'`-keyed, negative-controlled); 2D `drawFirepit2D` real-scale ring/square + warm flickering inner glow; 450 mm hit radius in `hitFixture`. NOT a `warm_hands` anchor (that anchor needs a hearth front; a radial pit has no front — noted follow-up). Gallery hand-lists updated (31 light kinds since the vanity family, guard-parity verified). **ON-only meshes (flames/ember/core) carry the click `ud` (2026-08-09)** — they occlude the tagged rim from most angles; the body-Group backstop tag already resolved clicks (probe-verified), the direct tags are hardening; sibling audit: the fireplace's flames live inside its full-carcass hit box (pinned), heatlamp's ON adds no geometry. NB the user report behind that audit ("can turn a firepit on but not off") was actually the avatar-interaction system relighting it — see USER-TOGGLE IMMUNITY in the avatar-interactions section. Test `firepit-test.html` (`FIREPIT PASS 65/65`).
  - **Vanity family (2026-08-09, user-requested "lights for bathroom vanities")**: `vanity_bar` 💄 (3-globe bar), `vanity_hollywood` 🎬 (5-globe marquee strip), `mirror_light` 🪞 (backlit mirror — panel + emissive rim halo). Wall-plane fixtures: flush-snap on drop + move-release via the pure `snapVanityToWall` (floodlight pattern, offset `WALL_HALF + 15` = 65, `isVanityLightKind` gate, no ganging); NO floor pool (join sconce/inground in the pool-skip list — they light the person, not the floor); follow the HA color/brightness normally (never forced warm — a blue bulb renders blue, pinned); globes stand off the plate on short arms (a globe hung under a 30 mm plate at the 65 mm offset buries in the wall — caught by test). **`LIGHT_KIND_HEIGHT_DEFAULTS` (geometry.ts) is the FIRST per-kind mount-height table**: `lightHeight(l)` now also reads `l.iconKind` — vanity_bar/hollywood 1950, mirror_light 1700, everything else byte-identical 2500 (NB pre-existing divergence left alone: the sidebar Height placeholders suggest per-kind values for under_cabinet/wall_sconce/step/flood/exhaust_wall that the renderer does NOT honour — migrating them into the table is a noted follow-up). Test `vanity-light-test.html` (`VANITYLIGHT PASS 105/105`).
...
Beyond the classics above there are casework kinds (coffee_table, tv_stand, dresser, nightstand, wardrobe, ottoman, stool, plant, counter, island, cabinet), **appliances** (fridge, stove, dishwasher, washer, dryer, microwave, tv — `cat: 'appliance'`, spec-sheet default sizes), **bathroom** kinds (toilet, sink, bathtub, shower — `cat: 'bathroom'`), **countertop/fitness** kinds (`coffee_maker`, `toaster` — `mountable`; `exercise_equipment`), and **home-theater** kinds — speakers/sub/center under `cat: 'theater'` (`isSpeakerKind`). **The over-inclusive default `furniture` cat was SPLIT 2026-08-07 (user-requested)**: `FurnitureCat` += `seating | tables | bedroom | storage | stairs | decor` and every built-in kind now carries an EXPLICIT cat — seating (chair family, sofas, ottoman, stool), tables (table/desk/coffee_table/counter/island), bedroom (beds, nightstand, dresser, wardrobe), storage (bookshelf/tv_stand/cabinet), stairs (the stairs family + riser_platform), decor (rug/block/exercise_equipment); theater_recliner/recliner_row3 moved to `theater`. **`plants` cat ("Plants & trees", 2026-08-09, user-requested "put these into their own categories in the side bar")**: exactly TEN kinds — `plant` (ex-decor) + `tree/pine_tree/oak_tree/birch_tree/palm_tree/willow_tree/spruce_tree/bush/flower_bed` (ex-outdoor); audited — NO runtime gate keys on `'decor'`/`'outdoor'` (all cat gates are `'appliance'`-style) and kind-based predicates (`isTreeKind`/`isDroopPlant`) are cat-independent, so the move is UI-classification only. Surfaces: `_kindOptions` optgroup after Decor; a toolbar Plants tab (🪴 — `CAT_GLYPH.decor` changed 🪴→🖼 to break the collision, commented) before Outdoor; gallery `CAT_PAGE.plants = 'outdoor'` (9 of 10 lived there — `plant`'s GIF URL moves furniture→outdoor page on the next gallery regen, accepted); AND the sidebar PLACED-ITEMS list split three ways — `_furnitureSection()` renders THREE `_section`s (Furniture → Plants & Trees (slug `plants`) → Appliances (slug `appliances`)), membership via `_furnSectionSlug` = `furnitureCat(resolveFurnitureDef(...))` (never `FURNITURE_KINDS[kind]` — vehicle/custom fallback kinds would mis-bucket; `!== 'appliance'`-ladder, never a positive `cat === 'furniture'` test), full-array `indexOf` idiom kept, all three render whenever the floor has any furniture, `SECTION_SLUGS`/`IDENTIFY_GROUPED_SECTIONS` gained both slugs, and `_autoExpandActive` + Alt+click identify route to the OWNING section (only that slug carries the active id in the snapshot). `'furniture'` stays in the union ONLY as the legacy/custom-object fallback (`furnitureCat()`'s `?? 'furniture'`; its sidebar optgroup renders only when something resolves to it). The 9 runtime layer/hash gates all test `=== 'appliance'`-style (audited — NEVER write a positive `cat === 'furniture'` test, commented at the union); the gallery's `CAT_PAGE` maps all six back to the ONE furniture page (distinct in-page group headings — six new pages would have orphaned every published `media/furniture/*.gif` URL). Toolbar: the Furniture tab became FIVE tabs (Seating/Tables/Bedroom/Storage/Decor) and the stairs-cat cards joined the existing Structure tab. **Bed SIZES (2026-08-07, user-requested)**: `bed_twin` (990×1910, 1 pillow) / `bed_full` (1370×1910, 2) / `bed` relabeled "Bed · queen" (id unchanged — back-compat; 2 pillows, PILLOW build bit-exact via `bedPillowLayout`'s 21/22 ratio; **dims CORRECTED 2000×1500 → 1524×2032 on 2026-08-09, user-reported** — the relabel had carried the legacy generic-double footprint, so the queen rendered sideways/short with the headboard on the long edge; both renderers are dim-generic, placed pieces keep stored dims; the demo library has since been regenerated onto canonical dims — re-measured 2026-08-19, all 19 `bed` instances are 1524×2032 and the last off-spec beds (two `bed_twin` at 900×1900 in starship) were corrected then) / `bed_king` (1930×2030, 3), all cat bedroom; **`isBedKind` (geometry.ts) is the canonical membership** for every former literal-`'bed'` predicate (lie lanes, covers, nav-occupiable exemption, sharedBedCovers UI — the isStairsKind precedent); lie-lane capacity auto-scales via width. Test `bed-sizes-test.html` (`BEDSIZES PASS 75/75` — §H pins h > w for every bed kind + real-world nominals within 25 mm; NB `bed-test.html` is a PRE-EXISTING manual visual harness — don't overwrite it). **`resolveFurnitureWallCollision` is PUSH-OUT-ONLY (2026-08-07, user-reported "bed sucked into the wall")**: it acts only when the near edge overlaps the keep-out band (< FLUSH 55 mm from the wall axis) and pushes out to exactly FLUSH — the old magnet also PULLED any piece within ~150 mm of the face flush, teleporting deliberately-gapped placements; dropping ON a wall still yields flush via push-out, so flush stays one gesture away; **STAIRS-family pieces are EXEMPT from the push-out and Alt = free placement skips the drop/release resolvers entirely (2026-08-09, user-reported "extend the lower stairway under this room and it will not allow me even while holding alt")** — a flight legitimately runs under walls/rooms into its shaft (physical.mjs `wallCollidable` always exempted stairs; the editor now agrees), and Alt (the align/wall-snap modifier) now also suspends `snapStairEdges` + the wall push-out + the seat tuck at BOTH the drop and the drag-release sites (`snapFurnitureToSurface` stays live under Alt — mounting is a semantic, not a snap) (align-guides §17 pins all scenarios incl. §17f–h stairs/Alt, 162/162; floorplans lib.mjs's settleWalls is its own push-only replica — regen verified exportedAt-only). **Six CHAIR kinds (2026-08-07, user-requested "styles for different rooms and uses"; all cat seating, real `seat` heights, NO `activity` — activities come from the HOST table/desk via seat capture, exactly like `chair`)**: `armchair` (club chair, seat 420), `office_chair` (5-star base + casters + gas lift, 480), `bar_stool` (750 — the tall seat flows into `SitSpot.seatY`, the `seatYeff` shoulder-clearance IK handles the perch; keeps `frontArrow` unlike stool/ottoman — aiming at a counter is useful), `wingback_chair` (wing panels, 430), `folding_chair` (X-fold tube frame, 440), `gaming_chair` (racing bolsters + proud accent stripes, 470; shares the office 5-star base build). The seat-capture pipeline has NO kind list — `resolveSeatTableCollision`/`carryTuckedSeatsWithTable`/SitSpot registration all key on `def.seat` presence, so new seat kinds ride along automatically (counters/islands are deliberately NOT activity hosts — only eat_at_table/work_at_desk tables capture). Builder meshes carry test-hook tags (`chairStarLeg`/`chairCaster`/`chairWing`/`stoolFootring`/… — the mailFlagArm idiom). Test `chairs-test.html` (`CHAIRS PASS 129/129`). `furnitureCat(def)` groups the sidebar kind dropdowns into optgroups. A def with `seat` set is **sittable** (humanoid seating anchors — see below).
...
`FurnitureKindDef` (geometry.ts) carries the Sims metadata: `activity?` (`ActivityKind` this piece anchors — see "Activity system"), `surface?` (counter-height top other pieces can sit ON), `mountable?` (sits on a surface piece, not the floor), `frontArrow?` (show the 2D front chevron on select; default on, off on symmetric kinds). **Mountable auto-snap**: dropping/moving a mountable piece near a `surface` host lands it on the top (`elevation` = host top; `mountOnId` records the host). **Live-parenting**: moving a `surface` host carries every furniture with `mountOnId === host.id` — the `furnMove` mousemove case translates them by the host's exact per-frame delta (captured `furnHostPrev`, applied AFTER the align-snap so they stay glued), and on release `snapFurnitureToSurface` re-settles each mounted piece's elevation/mountOnId. LOCKED mounted pieces do NOT follow (mount-on-host is not a frame change like `translateFloorContent`); they keep their `mountOnId` and re-snap on their next drag. Similarly, moving a table/desk (`activity` `eat_at_table`/`work_at_desk`) carries the chairs (seat-bearing kinds) that were tucked to its OLD position — `seatBelongsToTable` (geometry.ts, host footprint inflated by `TABLE_CARRY_MARGIN_MM` = 450 mm) selects them, they translate by the table's move delta then re-`resolveSeatTableCollision`, so a dining set moves as a unit (locked chairs stay put). **Both are WALL-AWARE since 2026-08-09 (found by the floorplan audit: a bedroom desk's capture box crossed a partition and silently re-aimed a hall-bath TOILET in the next room — 2 real instances in the shipped library)**: each gained a **TRAILING OPTIONAL `walls`** param (`seatBelongsToTable`'s 9th, `resolveSeatTableCollision`'s 5th) — **ABSENT ⇒ byte-identical** (pinned differentially against a pre-change build over 169 662 cases, the differ itself negative-controlled), the stale-caller idiom, so the floorplan scripts kept their exact behavior until opted in separately. The probe is the pure `seatWallBlocked` = `segCrossesSolidWall(walls, [], [], hostCentre, seatCentre)` — **the EMPTY doors/windows arrays ARE the design decision: a DOORWAY is OPAQUE to a capture** (a chair across an open doorway is not "at" the table; contrast the robot/avatar consumers of the same helper, which legitimately pass through doors); invisible walls stay passable, fences/hedges/railings block. Documented limit: a proper-crossing test misses a host whose CENTRE lies exactly on a wall centerline — only reachable under Alt free placement, where check 10 already flags it. Alongside it `NON_TABLE_SEAT_KINDS`/`isTableSeatKind` (the `STAIRS_KINDS` precedent) exclude `toilet` + `swingset` — seat-bearing pieces whose `seat` exists only for the piece ITSELF (a toilet even carries its own conflicting `activity: 'toilet'`; a 2800×1600 swingset sits beside a yard `picnic_table` with no wall between) — applied at the canvas-interact CALL SITES, never inside the geometry helpers, so the set stays inert for anything that hasn't opted in. Accepted consequence: a dropped toilet is no longer shoved out of a desk footprint (the wall push-out still runs). Test `seat-wall-test.html` (`SEATWALL PASS 32/32` — §A the byte-identity differential, §B/§C the guard incl. the doorway matrix and a real `small-bungalow.json` case, §D a derived 24-seat-kind exclusion sweep, §E end-to-end through the REAL `onCanvasMouseUp`; every assertion negative-controlled in place). Both are release/per-frame moves only, never rotate (furniture has no canvas rotate handle). The 2D front **chevron** draws on the selected piece toward local **−Z** — the FUNCTIONAL front (cabinet doors, screens, seat openings, humanoid faces), which is the opposite edge from the backrest/headboard (+Z) decorations.
...
the toolbar **Vehicles** tab (MERGED 2026-08-06 — one always-present tab, id
`'vehicle'`: car + ev_charger furniture kinds first, pack models after; see
"Visual placement toolbar") arming runtime `Planner.pendingVehicleModelId` (mutually exclusive with
...
(dim) instead of the kind dropdown for vehicle pieces. **Display naming
(2026-08-06, user-reported "vehicles are labeled block")**: the sidebar furniture
LIST row, the Label-input placeholder and ruler end captions all resolve through
`resolveFurnitureDef` (never `FURNITURE_KINDS[kind]` — a vehicle piece keeps
...
### Avatar motion fluidity (2026-08-19, user-requested "more fluid movement")
Measured first (`test-pages/avatar-fluidity-test.html`, `FLUIDITY PASS 20/20`,
deterministic under `mulberry32(20260819)`); **three of the four plausible
culprits were REFUTED by measurement and one unsuspected cause dominated** —
the numbers are recorded here so none of them is re-proposed.
- **THE defect: the blocked-cell carrot snap.** The `gap <= 600` graze branch in
  `updateTargets` ASSIGNED a nav-cell centre straight into the spring's setpoint
  (deliberately leaving `navX/navZ` + `nvx/nvz` alone, commented as "a mere
  corner graze mid-walk is small"). Measured, that graze is a **94–232 mm step
  input against a 26–32 mm/frame walking rate**: nav jerk median 174 417 mm/s³
  vs a **33 mm/s³** walking baseline (**5230×**), owning **8 of the 8** worst
  frames of a 300 s run, none of them corners. A spring cannot absorb a step in
  its own target. Fix = **rate-limit, never teleport**: the carrot now WALKS to
  the same cell at `max(1800 mm/s, seek pace)`. Cell choice, region/LOS/A*
  semantics and the `gap > 600` hard-reset branch are byte-unchanged (that one
  is a deliberate "rig is badly wrong" re-seat and DOES hard-set nav). Moving
  nav by the same delta instead was REJECTED — it converts the jerk into a
  232 mm instant teleport of the drawn body (~7 m/s apparent at 30 fps). Carrot
  direct assignments 23 → **0**; step max 191 → 48.8 mm/frame; nav jerk
  p99/median **1557 → 417**; the carrot's blocked-cell streak is bounded and
  asserted ≤12 frames (the invariant the snap exists for).
- **Facing stays FIRST-ORDER. Do not "fix" it.** The hypothesis that yaw needs a
  critically damped spring was measured and is WRONG in both halves; four real
  builds were compared over the same 300 s run (peak yaw accel rad/s², lower
  better): shipped first-order + hard 50 mm/s gate **52.9**, vs 67.7 / 79.0 /
  121.8 / 152.6 for every alternative. (a) An angular spring answers a heading
  error with `α = ω²·e`, so it AMPLIFIES exactly the errors that occur and
  builds momentum — facing lag 32° → 130°. Second-order is right for POSITION
  (`_springNav`) because the carrot never steps; a heading target genuinely
  does. (b) The nav→carrot lead vector is NOT a better source: it collapses
  through zero and REVERSES while a figure decelerates (carrot stops first, the
  spring-trailed body coasts past), injecting 180° flips in the same 200–235
  mm/s band the worst events live in. (c) Hysteresis loses both ways. NB yaw is
  ~15 % worse after this batch purely because item 1 changed the TRAJECTORY (the
  rig walks different corners) — the facing code is behaviourally identical
  apart from exact easing. Rationale is duplicated in-code so it isn't retried.
- **Exact easing `1 − e^(−k·dt)`** replaces `Math.min(1, dt*k)` at the MOTION
  sites (both velocity low-passes k=4, amp k=6, groundY k=8, anchored-nav k=3,
  facing) — the documented wall-cutaway lesson. Facing divergence vs 120 fps:
  29.7° → **14.0°** @10 fps, 9.4° → **6.0°** @30, 3.1° → **2.0°** @60. Below
  10 fps the 0.1 s dt clamp dominates, so @6 fps is slightly worse (60.2 →
  65.8°) — accepted. **Pose blends (`sit`/`act`/`lie`/`idleBlend`/`privacy`) and
  the spawn/despawn scale eases were deliberately NOT converted**: they set
  CAPTURE and LIFECYCLE timing pinned across many suites (`despawn-test` pins
  `alpha@5s`), and the measured payoff was entirely in facing/nav/amp.
- **Cadence now derives from the EASED amp** (`persAmp` divided back out so walk
  personalities survive; envelope 0.4–2.5× `cadenceNom`; steady state returns
  `cadenceNom` exactly). Foot/ground speed ratio p99 **1.59 → 1.00**; frames
  with >25 % foot skate **10.76 % → 0.07 %**.
- **`_separateRigs` is a FORCE, not a position write**: rate-limited carrot
  displacement + a repulsion ACCELERATION into `nvx/nvz` ∝ overlap depth.
  Near-pass jerk p99 3 428 391 → **346 532** (ratio vs free walk 318 → 35).
  Min gap 380 → 251 mm is EXPECTED — the old 380 was held only by the teleport
  being removed; two rigs on a head-on course now brush for ~0.3 s instead of
  popping apart.
- **REFUTED, do not revisit**: the gait-phase freeze (real mechanically — phase
  advance after stop is exactly 0 rad — but amp has already eased to ~0.07 rad
  before the `walking` boolean flips, so legs sit within 4.6° of neutral and
  settle to ~0.8° in 1 s: invisible); `_seek`'s clamps and the `far > 1200`
  catch-up (0 % / 0 % / 0 % engaged in every run, max `far` 185 mm against the
  1200 gate); and corner/waypoint arc-blending (real, but the spring already
  absorbs 44–168× of it and it yields 7× less nav jerk than the carrot snap for
  far more invasive surgery).
- Every one of the 20 assertions is **negative-controlled by a real broken
  build**, not by inspection: a full `git stash` revert fails 11/20; a
  deliberate-breakage build trips the rest; `SEP → 0` trips the min-gap pin.
...
### Mechanical & utility appliances (water heater / HVAC plant / pumps / 3D printer / network rack)
Eleven `FurnitureKind`s, all `cat: 'appliance'` (they ride the appliances layer + the three-view
...
`recirc_pump`, `printer_3d` (`mountable`), + **`network_rack` (2026-08-05,
peripheral-fixtures research)**: 600×500×1200 home 12U cabinet OR NAS tower
(`Furniture.rack {problemEntities[], cpuEntity, tempEntity, shape}` — nested
like mailCount/evCharger); pure `rackHealth` aggregate (problem > update >
ok > unknown; `update.*` 'on' = firmware amber, other domains' 'on' = fault,
unknown/unavailable contribute NO opinion — the heat-map honesty rule) drives
ONE emissive LED bead (3D) + a corner LED pulsing only on problem (2D), both
views through the same resolver; DISPLAY-ONLY (fixtureId tag only, never
clickable — the vehicle precedent), excluded from the generic in-use LED, the
DERIVED health band (not raw member states) folds into the appliance hash so
a flapping diagnostic can't thrash rebuilds; bound ids config-path. Test
`rack-test.html` (`RACK PASS 44/44`). Gate helper `isMechanicalApplianceKind` (+ `isPumpKind`);
...
3D-side). **Sink TOPS rebuilt (2026-08-07, user-reported "intersecting geometric
shapes instead of a cohesive top with a bowl")**: the old counters were FOUR
abutting rim boxes (shared face planes hatch under flat toon; each box's
inverted-hull shell z-fought its neighbor's into black seams) and — worse — on
4 of 5 kinds the solid body's top face sat ABOVE the recessed bowl floor, so
the bowl was never actually visible. Now: `addSinkDeck` builds ONE
`ExtrudeGeometry` slab whose opening is a real HOLE (the wall-with-window
idiom); `addOvalBowl`/`addRectBasin` stand walls `SINK_BOWL_LIP` (10) outside
the hole and bury their tops `SINK_WALL_BURY` (6) under the deck top; all basin
parts are `outlineSkip` (their shells punched up through the deck as bars);
bodies became SHELLS with basin shafts (pedestal's cap was hiding its bowl;
utility's basin walls now ARE the visible tub sides; kitchen_sink = one slab
with TWO holes — the deck strip between them IS the divider). Faucets, water
rigs, wash_hands anchors, `surface:true`, footprints all unchanged. Test
`sink-test.html` (`SINK PASS 84/84`). NB pedestal's splayed foot (r 115) is
fully inside the column base (r 130) and never renders — known cosmetic,
untouched.

### Shared avatar props (chores, snacks, umbrella, fetch)
Research `docs/research/shared-props.md` (+ one pinned delta below).
`PROP_DEFS` (three-renderer): 14 props (parasol added 2026-08-05 — the
umbrella's structural sibling, class-3 passive weather garment for ALL rigs
incl. radar/BLE, gated by the pure `uvParasolWanted(uvIndex, condition)` in
geometry.ts: UV ≥ 8 AND condition sunny; RAIN WINS — the umbrella checks
first and the parasol predicate refuses under rain;
`_wantActiveWeatherProp` releases by the ACTIVE prop's own predicate;
`ActivityContext.weather.uvIndex` optional/stale-chunk-safe) —
vacuum_cleaner, broom, dish_towel,
...
already right). Test `props-test.html` (`PROPS PASS 125/125` — section 15b drives
...
(`AREABIND PASS 116/116`). The anchor is a world-mm point; `resolveRoomForPoint(rooms, loops, x, y)` (geometry.ts) maps it to whichever **live closed wall loop** (`closedWallLoops`) contains it, so room identity survives wall edits. `resolveRoomForPointFuzzy(rooms, loops, x, y, probeMm = 250)` is the boundary-tolerant variant: it tries the exact point, then probes a ring of offsets (order: +y, -y, +x, -x, then the four diagonals) and returns the first room hit — needed because doors, windows, and flush wall-mounted fixtures (switches, fireplaces) sit exactly ON a wall line, which `pointInPolygon` excludes, so an exact resolve would drop them into "No room". The sidebar `_groupByRoom` bucketing uses the **fuzzy** resolver for all item kinds; a boundary item touching two rooms goes to whichever probe lands first (deterministic, acceptable). Created via the sidebar "Rooms" section: **+ Add room** then `placingRoomId` arms a click-to-anchor on the 2D canvas — the room is created **unnamed** (no prompt); `roomLabel(rm)` (geometry.ts) supplies an italic/dimmer "Unnamed room" placeholder until the sidebar input names it, and an anchor outside every loop draws an amber "not enclosed by walls" marker at the anchor in 2D. Labels render **AT THE ROOM'S ANCHOR (2026-08-01, user-requested "labels don't center; make the placed marker hold the label position")** in 2D and as a `THREE.Sprite` in 3D (same world x/y; the 3D sprite is **BOTTOM-anchored** — `sprite.center.set(0.5, 0)`, the bubble-tail idiom — since 2026-08-06 (user-reported "labels cut into the floor"): the 360 mm billboard sits only ~50 mm above the slab, so a CENTER anchor hung its lower half below floor level and the opaque slab depth-clipped it at shallow camera angles; bottom-anchoring is exact at every angle — pinned by vertex-insert L4), both gated by the `labels` layer — and the label is DRAGGABLE in edit+Select (`roomAnchor` drag; hit via the `roomLabelHalfPx` px-extent export, the envChipHalfPx idiom, cleared outside the labels gate = hidden layer undraggable; runs after every fixture hit and BEFORE pool/ground/void body hits so a fixture under the label still wins). Dragging the label re-homes the room to whichever loop holds the anchor (per-room flooring + bindings follow — the room IS where its anchor is); outside all loops → the amber not-enclosed state live. The heat-map temp label deliberately stays at the loop CENTROID (room-wide datum; would collide with the name). The sidebar re-place latch (`placingRoomId`) is unchanged. Room names feed the activity + bubble systems — a name containing the substring **`kitchen`** (case-insensitive) gates the snack/coffee bubbles, and the seated-person's room scopes TV watching.

**Per-room flooring + no-floor-outside-rooms + outdoor binding (2026-08-01, user-directed)**: `Room.floorColor?`/`Room.floorTex?` (item-level; ladder room → `Floor.look3d` → `Store.scene3d` → defaults, `null` = inherit; pure `roomsByLoop`/`roomFloorLook` in geometry.ts) give each room's wall LOOP its own floor look — 3D: per-loop slab material via a `color|tex` cache seeded with the floor-wide `floorMat` (occupancy `occMat()` still WINS while occupied, room look returns on clear); 2D: `drawFloor` fills EACH loop with its room look (`drawFloorTexture` gained an optional px `region`, tiling anchored at world (0,0) so grain is continuous across rooms) and — the headline — **fills NOTHING outside the loops** (the canvas background shows; yardFill/ground areas paint as before via `drawGroundAreas`; boundary stroke, grid, bg-image clip and dim labels stay full-rect; no loops → classic full-rect fill; NAV UNTOUCHED — the yard stays walkable, NAVPARITY 70/70 pins it). 2D wall loops now come from ONE module-level single-slot cache `floorLoops()` (canvas-render; keyed `floorId | wallCount | FNV-1a wall-point hash` — NOT configRev, a wall vertex DRAG mutates walls without save(); 5 former per-frame `closedWallLoops` call sites share it — net perf win). **Bindable outdoors**: `Floor.outdoor?: OutdoorArea` (`{name?, haAreaId?}`, in `repairFloor`'s field list — storage.ts; the sidebar "Outdoors" pseudo-row pinned at the BOTTOM of the Rooms list auto-prunes when both fields clear, the look3d idiom) + `GroundArea.haAreaId?` (item-level, per-area "HA area" dropdown in Ground/Yard). The pure `resolveAreaBindingForPoint(floor, loops, x, y)` (geometry.ts) ladder: (1) fuzzy ROOM hit ends the ladder even when unbound; (2) else SMALLEST bound containing GroundArea (by |polygonArea|, hidden/unbound skipped; label name → area name → kind); (3) else `floor.outdoor` when outside every loop AND `outdoorConfigured` (non-blank name or bound area; label name → area name → "Outdoors"); (4) null. `_areaFilterForPoint` (sidebar) runs the full ladder so env/thermostat/occupancy pickers scope outdoors + ground areas too; `_groupByRoom`'s "— No room —" bucket takes the outdoor LABEL when configured (the `/none` collapse key is UNCHANGED — persisted keys never migrate); `_areaOptions` widened to any `{haAreaId}` holder. Tests: AREABIND 116/116, LAYERS2D 84/84 (§8 paint matrix incl. live wall-edit cache), TERRAIN 119/119 (§L per-loop 3D materials).
...
- **Ambient idle-activity anchors** (renderer-synthesized, NOT authored on furniture defs — the kinds `warm_hands` / `gaze_window` live in a renderer-local `ExtActivityKind`, not `types.ts`): `updateFloor` also pushes into `_activityAnchors` a **`warm_hands`** anchor ~700 mm in front of each fireplace-`iconKind` LIGHT's opening (opening dir = `(sin lr, −cos lr)` scene, `lr = lightRotation·rad`; `facing = atan2(offsetDir.x, offsetDir.z)`, `standOff` 0, `lightId` stamped) and, for the **6 largest windows per floor**, a **`gaze_window`** anchor 600 mm inside the room (interior side = the window normal whose inset point falls in a closed wall loop, floor-center fallback; faces the glass). Both ride the standard PHASE4 standing-dwell machinery (`>1.2 s`, RAW pos). **Activity anchors are SINGLE-OCCUPANCY since 2026-08-06 (user-reported "avatars gather near windows like they are trying to escape" — root-caused on the user's real plan: anchors had NO occupancy rule (unlike `seatClaims`), the roamer wander centre was PINNED to the floor centre which sat ~540 mm from one window wall, and that window's `gaze_window` anchor was the only in-range candidate → 7 of 8 roamers piled on one anchor, 59 % of all rig-frames within 1.5 m of one window; NOT a windows-block-nav regression — the pre-fix nav measured the same 52 % crowd, it just leaked outdoors)**: `_anchorClaims` (the standing twin of `seatClaims`, rebuilt per frame from live rigs) + `AiState.anchorGoalId` reserves an anchor from goal-pick through capture — one rig per hearth/window/appliance face; ambient `gaze_window`/`warm_hands` anchors whose stand cell is nav-blocked are DROPPED at build. AI goal picks resolve reachability from the RIG's region (`ai.x/ai.y`, not the search anchor), reject cross-region snaps, and an A* failure RE-PICKS (`_aiNoGoal`) instead of straight-lining the carrot into a wall. The `warm_hands` anchor is built regardless of on/off; it's ACTIVE only per-frame when `ctx.fireplaceOn[lightId]` (from three-view's fireplace-light effective state) — checked in the dwell scan + engagement release, NOT baked at build, so it deactivates the instant the fire goes out. Poses: warm_hands = both arms forward toward the fire (shoulder ≈1.1, elbows ≈0.5, slight forward lean); gaze_window = still, one hand drifting up to the pane (shoulder ≈0.7–1.05), other arm relaxed.
- **Solo (standing) activities** (`PHASE4_ACTIVITIES`: shower/bathe/toilet/wash_hands/load_dishwasher/make_coffee/forage_fridge/exercise/warm_hands/gaze_window): a target dwelling ~1.2 s near an anchor eases an `act` 0→1 pose blend and turns to face the appliance. **Orientation gate (2026-07-30, user-reported "looking into the fridge from behind")**: pieces with a functional front (`hasFunctionalFront(def)` = `frontArrow !== false`; `frontVectorPlan`/`inFrontHalfspace` in geometry.ts) gate anchor CAPTURE on the rig's RAW position being in the FRONT halfspace (`ActivityAnchor.frontNx/frontNz` — the standing analogue of SitSpot front-only entry), the STAND point is out the functional front (the old `facing = −rot` computed local +Z = the BACK for every appliance — sign fixed to `pieceYaw + π`), `_advanceApplianceDoors`' proximity trigger requires front-halfspace, and `_aiPickGoal` aims `INTERACT_FRONT_STANDOFF` 700 mm in front of oriented interactive items (fire gate front-only; cooldowns arm regardless — no retry storm). Symmetric pieces (`frontArrow: false` — plants, tables' seat-driven paths, bins, stools…) stay radial; `tend_plant` approachable from any side by authored data. **`tend_plant` pose rebuilt (user-reported)**: static forward lean + one-arm tending oscillation (shoulder/elbow, ~3.9 s, idleOffset-desynced), NO rhythmic pelvis motion (the old ±0.05 rad root-pitch cycle rocked the hips ±40 mm — the entire old animation); `browse_bookshelf` inspected, already static, regression-pinned. Test `orient-interact-test.html` (`ORIENT PASS 23/23` — incl. pelvis-travel <6 mm vs hand-travel >40 mm world-geometry pose pins). `ENTITY_GATED_ACTIVITIES` (load_dishwasher, make_coffee) engage only while the bound entity is on; `warm_hands` gates on the fireplace light being lit (see above).
...
- **Settings drawer** (`<diorama-settings-drawer>`, ~560 px, tabbed:
  Connection | Display | Weather | Avatars | Integrations | Floor Plan (slug `data`); non-edit
...
- **Config** (`Store.weather: WeatherConfig`, optional/opt-in): `source` (`'entity' | 'sensors' | 'openmeteo' | 'demo'`), `entityId` (weather.*), `sensors` (`{precip, windSpeed, temp, lightning}` entity ids), `zip`/`lat`/`lon`/`placeLabel` (Open-Meteo location; zip geocoded once → lat/lon cached), `chip` (default true), `effects3d` + `affectLighting` (default true — **persisted now, consumed in W2**), `demo?: DemoWeatherConfig` (see the demo-source bullet). In `_loadFromHa`'s explicit field list; no per-floor fields.
...
- **Demo source (2026-08-05, user-requested — hand-authored weather)**: `source: 'demo'` + `WeatherConfig.demo?: DemoWeatherConfig` (all-optional: `condition`, `tempC`/`apparentC`/`humidity`, `windKmh`/`windBearing`/`windGustKmh`, `cloudCoverage`/`visibilityKm`/`uvIndex`, `rainSoon`, `forecastCondition` (anticipation bubbles), `moonPhase` (8 HA states; absent = follow `moonEntity`), `sunElevationDeg`/`sunAzimuthDeg` (absent = real sun), `alertSeverity`; rides the whole-object `weather` passthrough). Pure synthesis in weather.ts: `demoWeatherNow` (defaults sunny/22 °C/8 km/h, clamps, `isDay` from demo sun elevation when set else condition, sunny↔clear-night re-gated; `stale:false`, `label:'Demo'`), `demoSunAltAz(weather)` (**THE single sun-override helper** — non-null only under demo + both fields finite), `demoMoonPhase`, `demoWeatherAlerts` (one synthetic alert → the beacon/chip badge; the alert-entity path is bypassed under demo). Planner: `_recomputeLocalWeather` demo branch — no poll, no network, offline-safe, never stale; forecast ARRAYS stay null (chip strips hide — honest). **Sun-override consumers (all follow the demo sun — enumerated so nobody adds an eighth silently)**: three-view `_weatherFxState` sun stamp (scene sun light, sky sun disc, star gate, `_keyWeather` buckets; azimuth is a compass bearing through the existing `compassToPlanDeg` — no second convention), three-view `_keySolar` → `resolveSunPlan(..., override)` (solar.ts stays zero-import; `SunSource` gained `'demo'`), 2D `drawSolarPanels`, the sidebar solar readout, `_effectivePreset` → `resolveScenePreset(..., sunElevOverrideDeg)` (clock-mode lighting genuinely flips night at a demo −10°), the three-view moon-phase resolve, and `resolveTimeBucket`'s ActivityContext read (avatar kitchen morning/night tiers follow the demo night). `resolveScenePreset`/`resolveTimeBucket` gained optional trailing overrides (absent = byte-identical). Settings ▸ Weather: "Demo (hand-authored)" radio swaps in the authoring block (imperial-aware temps; wind stays km/h; entity/station/Open-Meteo + alert-entity rows hide). Tests: weather-test demo matrix + a real-Planner/fake-HaApi section with a `window.fetch` counter pinning ZERO network calls; solar-test §9 (demo sun reaches the array, incl. a 2D painted-depth end-to-end pin).
- **Chip** (`<diorama-weather-chip>`, light DOM via `define.ts`): mounted ONCE in `app.ts`'s shared canvas container so it overlays both 2D and 3D without a duplicate interval. Glyph + temp (respects `store.imperial` for °F) + place/entity label; dims when stale; hidden when `weather.chip === false` or no source resolved. Non-interactive except an edit-mode click → `open-weather` event → the Settings drawer's Weather tab. **DC-C position/content/forecast** (all on `WeatherConfig`, additive, whole-object through `_loadFromHa`): `chipAnchor` (`'tl'|'tm'|'tr'|'bl'|'bm'|'br'`, default `'br'` = the legacy bottom-right; TOP anchors clear the 3D view-controls bar — at `top:8px`, ~30 px tall — via a `barOffsetPx` default 44), `chipCustom {x,y}` (px offsets from the anchor's edges, WINS over the pure anchor), and `chipContent {apparent?, humidity?, wind?, uv?, hourly?, daily?}` (extra rows + forecast-strip entry counts, 0/absent = hidden). The chip stays a compact pill until any content/forecast is enabled, then grows a panel: current row + optional feels-like/humidity/wind rows + a horizontal hourly strip (hour/glyph/temp) + a vertical daily list (day/glyph/hi-lo), reading `Planner.forecastHourly`/`forecastDaily`. Position CSS comes from the PURE `chipAnchorStyle(anchor, custom, barOffsetPx)` in `weather.ts` (shared with the test page); content defaults from `resolveChipContent` (counts clamped 0..12 / 0..7). Settings ▸ Weather tab has a "Chip appearance" block (`_weatherAppearance` in modals.ts): 6-anchor button grid + custom x/y inputs (+ Clear) + content checkboxes + hourly/daily count inputs. **Minimize-to-pill (2026-08-08, user-requested)**: a chevron on the current-conditions row (own stopPropagation target, the alert-badge idiom; shown ONLY when panel content exists — a bare pill stays byte-identical) collapses the grown panel to just the pill; DEVICE-LOCAL `localStorage['diorama:weather:min']` (toolbar-collapse idiom, never the synced store — kiosk tablets minimize independently), absent = expanded; an open ALERTS panel still expands a minimized chip (urgent > tidy). Test `weather-chip-test.html` (`WCHIP PASS 22/22` — a planner-shaped stub, no real Planner; recipe in the page header). Test page `test-pages/weather-test.html` (`WEATHER PASS 350/350`: WMO table, sensor heuristic, unit normalization, entity sun-gating, forecast extraction, chip anchor/content resolution, Open-Meteo forecast parse, alert parse matrix) — bundle `weather.ts` via `esbuild --bundle` like `trilateration.html`.
...
### Rotation pivots (2026-08-07, user-reported "a door should not rotate about an edge")
The SIDEBAR Rotation inputs for doors route through the pure
`rotateDoorAboutCenter(d, newRotationDeg)` (geometry.ts): a door's stored (x, y)
is its HINGE, so mutating `rotation` alone swept the panel about that end — a
180° flip teleported it to the far side. The helper re-anchors the hinge as a
DELTA (`x + (cos t₀ − cos t₁)·w/2`) so the span centre is invariant and
+180/+180 cancels (round-trip pinned ≤1e-9 mm — BIT-exact is unreachable in
doubles, `(x+d)−d` re-associates; measured worst 4.6e-13 mm; don't "fix" by
quantizing, reasoning at the helper; `rotation` itself IS bit-exact). Windows'
(x, y) is already the pane CENTRE (`rotateWindowAboutCenter` returns x/y
untouched — windows were never broken; `hitWindowEnd`'s 'a'|'b' misleads).
Doors are the ONLY end-anchored rotatable (swept: all 12 other sidebar rotation
inputs rotate about their stored centre/mount point). The CANVAS endpoint-drag
deliberately keeps pivoting about the grabbed end (that gesture IS "swing this
end" — commented at the drag site). door-kinds 131/131, chairs-test 137/137
(exercise_equipment gained its missing motor cowl — its deck and tower AABBs
were TWO disconnected components 291 mm apart, the floating-car-parts bug
class; 1 mm-touch-tolerance connectivity pin, negative-controlled).
...
- **canvas-2d `_resize` has a SAME-VALUE guard (2026-08-07, user-reported rhythmic 2D flicker — frame-diffed from their video: one fully-blank frame every 2.0 s)**: per the HTML spec ANY assignment to `canvas.width/height` wipes the bitmap, even to the unchanged value; `_onConfig` routes EVERY config emit through `_resize`, and on a live instance with periodic emits (mmWave firmware `number.*` slow-path sync ≈ every 2 s) the unconditional assignment blanked the canvas once per emit — when the wipe landed after that frame's RAF paint, the compositor presented a blank frame. Guard: assign only when the rounded px size actually changed. Verified via CDP screencast: forced 2 s emits → zero assignments, zero blank frames. Never remove the guard.
