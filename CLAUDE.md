...
composed) caps the log curve at the TRUE elevation angle, `dispY = max(yMinMm,
...
overhead still reads overhead) and elsewhere the display angle can only be ≤ the true one; the
`yMinMm` floor is the single exception, lifting distant low traffic clear of rooftops. (Before the
...
OSM line) whenever cloud + enabled + data. Tests: `flights-test.html` (`FLIGHTS PASS 335/335` —
